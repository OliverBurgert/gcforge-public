...
from geocaches.runtime_cache import invalidate_my_usernames_cache
...
def oauth_start(request):
    ...
    acct = UserAccount.objects.filter(id=acct_id).first()
    if not acct or acct.platform not in UserAccount.oc_platforms():
...
def account_validate_gc(request):
    """HTMX endpoint: validate the GC API token by fetching the user's profile."""
    if request.method != "POST":
        return HttpResponseNotAllowed(["POST"])

    try:
        from geocaches.sync.gc_access import get_gc_client
        profile = get_gc_client().get_my_profile()
