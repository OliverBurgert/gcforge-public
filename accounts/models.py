...
class UserAccount(models.Model):
    ...
    @classmethod
    def oc_platforms(cls) -> frozenset[str]:
        """OC platform ids (every non-GC platform in ``PLATFORM_CHOICES``).

        Canonical single source of truth for "is this an OC node?" checks
        (quota tracking, log-source matching, OKAPI node selection).
        """
        return frozenset(p for p, _ in cls.PLATFORM_CHOICES if p != cls.PLATFORM_GC)
