...
def fragmentation_info() -> dict:
    ...
    """
    # Use Django's own connection rather than sqlite3.connect(get_db_path()):
    # under the test runner the DB "path" is the in-memory URI
    # file:memorydb_default?mode=memory&cache=shared, which a raw connect on
    # Windows turns into an empty file literally named `file` in the cwd.
...
def restore_from_path(backup_path: Path) -> None:
    ...
    # The file swap above bypasses the ORM entirely, so none of the
    # process-level caches saw it happen — drop them all so the next request
    # rebuilds from the restored database instead of serving stale state.
    from preferences.models import UserPreference
    from preferences.context_processors import invalidate_connected_platforms_cache
