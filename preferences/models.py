...
class UserPreference(models.Model):
    ...
    # Process-level {key: raw value} cache — one SELECT * per process instead
    # of one SELECT per get() call (179 call sites, ~80 rows). Invalidated by
    # the post_save/post_delete signals below (covers set(), any other
    # save()/delete(), and QuerySet.delete()), plus explicitly after backup
    # restore (preferences/backup.py) since that replaces the DB file without
    # going through the ORM. See docs/architecture-review-2026-09.md §4.1
    # item 2 and docs/architecture-review-2026-09-workplan.md WP-01.
    _cache = None
    ...
    def _get_cache(cls):
        ...
        with cls._cache_lock:
            if cls._cache is None:
                cls._cache = {row.key: row.value for row in cls.objects.all()}
            return cls._cache
...
def _invalidate_preference_cache(sender, **kwargs):
    UserPreference.invalidate_cache()


post_save.connect(
    _invalidate_preference_cache, sender=UserPreference,
    dispatch_uid="userpreference_cache_invalidate_save",
)
post_delete.connect(
    _invalidate_preference_cache, sender=UserPreference,
    dispatch_uid="userpreference_cache_invalidate_delete",
)
