...
(gbOpen, CC-BY 4.0) when the user clicks Download for a country (both tiers
together, county best-effort), then stored locally under ``<db_dir>/boundaries``
so rendering — and offline location enrichment — stays offline afterwards.
Polygons join to the find set by name (``shapeName`` → cache ``state`` /
``county``), compared after light normalisation.
"""
...
_ADMIN1_SUFFIXES = (
    ...
    " kommune",               # Danish: Nominatim "Varde Kommune" vs polygon "Varde"
...
_ADMIN1_PREFIXES = (
    ...
    "county of ",
    "principality of ",
    "region ",                # Danish "Region Hovedstaden" (Nominatim) vs "Hovedstaden"
...
_COUNTY_ALIASES: dict[str, dict[str, str]] = {
    ...
    "DK": {
        # geoBoundaries names the capital municipality in English; Nominatim
        # returns the Danish "København" (city) / "Københavns Kommune".
        "kobenhavn": "copenhagen",
        "kobenhavns kommune": "copenhagen",
    },
}
...
def update_all() -> dict:
    """Re-download every boundary-tier country already in the manifest, using
    the current effective levels (auto-corrects countries whose override moved
    them to a different admin level since the last download). Always refreshes
    both region and county (county best-effort) — this also backfills county
    data for any country downloaded before both tiers became the default.
    Returns {key: count|error}.
    """
    results = {}
    # De-duplicate by ISO so we don't redownload the same country twice when a
    # stale entry exists at a different level.
    seen_iso = {info["iso2"] for info in _manifest().values()}
    for iso2 in seen_iso:
        region_key = _key(iso2, effective_level(iso2))
        try:
            results[region_key] = download_boundary(iso2)
        except (requests.RequestException, ValueError, RuntimeError, OSError) as exc:
            results[region_key] = f"error: {exc}"

        county_level = effective_county_level(iso2)
        county_key = _key(iso2, county_level)
        try:
            results[county_key] = download_boundary(iso2, level=county_level)
        except (requests.RequestException, ValueError, RuntimeError, OSError) as exc:
            results[county_key] = f"error: {exc}"
    return results
...
def status_summary() -> list[dict]:
    """Every downloaded boundary, grouped by country, for the Enrichment tab.

    Returns one row per country with any manifest entry — even one with no
    finds — since offline enrichment only cares what's on disk, not what the
    Dashboard's per-country choropleth table (which lists finds only) shows.
    """
    ...
    for info in _manifest().values():
        ...
        row = by_iso.setdefault(iso, {"iso": iso, "name": iso_to_name(iso)})
        if level == effective_level(iso):
            row["region_count"] = info.get("count")
            row["region_downloaded_at"] = info.get("downloaded_at", "")
        elif level == effective_county_level(iso):
            row["county_count"] = info.get("count")
            row["county_downloaded_at"] = info.get("downloaded_at", "")
