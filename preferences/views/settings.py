...
from .dashboard import _build_country_finds, _countries_without_finds
from .database import _list_available_databases, _build_profiles_context
...
def settings_view(request):
    ...
    from preferences.services import boundaries as boundaries_mod
    ...
    context = {
        ...
        "frag":                   frag,
        "db_stats":               db_stats,
        "profiles":               _build_profiles_context(db_path),
        "can_auto_restart":       gcforge_restart.can_auto_restart()[0],
        ...
        # GC access mode (official API vs. website/tRPC fallback)
        "gc_access_mode": UserPreference.get("gc_access_mode", "api_preferred"),
        "gc_api_available": gc_api_available(),
        ...
        "boundary_status": boundaries_mod.status_summary(),
        "dashboard_country_finds": dashboard_country_finds,
        "dashboard_other_countries": _countries_without_finds(
            r["iso"] for r in dashboard_country_finds
        ),
...
def save_prefs(request):
    ...
    # GC access mode
    gc_access_mode = request.POST.get("gc_access_mode", "")
    if gc_access_mode in ("api_preferred", "web_preferred"):
        UserPreference.set("gc_access_mode", gc_access_mode)
    # Icon set
    icon_set = request.POST.get("icon_set", "cgeo")
