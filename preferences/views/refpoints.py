...
def edit_refpoint(request):
    ...
    if rp_id and name and result:
        lat, lon = result
        rp = ReferencePoint.objects.filter(id=rp_id).first()
        if rp is not None:
            moved = (rp.latitude, rp.longitude) != (lat, lon)
            ...
            # queryset.update() fires no signal, so drop the now-wrong cached
            # distances here and let the next request schedule the refill.
            if moved:
                from geocaches.geo.distance_cache import invalidate
                invalidate(ref_point=rp)
    return _redirect_tab("reference-points")
...
def set_current_location(request):
    ...
    # The save signal already dropped the stale rows when the point moved;
    # refilling them takes seconds, so it happens in the task dock, not here.
    from geocaches.geo.distance_cache import schedule_recompute
    schedule_recompute(rp)
