...
def _clone_save_payload(data: dict) -> dict:
    """Shallow-clone a sync payload so save_geocache can mutate it safely."""
    kw = dict(data)
...
def _run_update(qs, action: str, kwargs: dict, *, task_info):
    ...
    elif action == "alc_only":
        return _alc_only(qs, task_info)
    elif action == "light_update_alc":
        return _light_update_alc(qs, task_info)
    elif action == "full_update":
        return _full_update(qs, task_info)
...
def _light_update(qs, task_info):
        ...
        try:
            ...
            for data in results:
                if task_info.cancel_event.is_set():
                    break
                try:
                    save_geocache(**_clone_save_payload(data))
                    updated += 1
    ...
    if oc_codes and not task_info.cancel_event.is_set():
        ...
        try:
            ...
            platform_codes = group_by_platform(oc_codes)
...
def _full_update(qs, task_info):
    """Full refresh: all fields, descriptions, waypoints, attributes, + recent logs."""
    import time
    from geocaches.services import save_geocache
    from geocaches.sync.base import SyncMode

    gc_codes = list(qs.filter(gc_code__startswith="GC").values_list("gc_code", flat=True))
    ...
    BATCH = 25   # smaller than client's internal 50 — more frequent progress updates
    PAUSE = 2.0  # seconds between batches to keep server load low

    logger.info("Full update starting: %d GC + %d OC caches", len(gc_codes), len(oc_codes))
    task_info.phase = f"Starting: {len(gc_codes)} GC + {len(oc_codes)} OC caches"

    if gc_codes:
        from geocaches.sync.gc_access import get_gc_client
        client = get_gc_client()
        for i in range(0, len(gc_codes), BATCH):
            if task_info.cancel_event.is_set():
                break
            batch = gc_codes[i:i + BATCH]
            task_info.phase = f"GC full update: {completed + 1}–{min(completed + len(batch), len(gc_codes))} of {len(gc_codes)}"
            try:
                results = client.get_caches(batch, SyncMode.FULL)
                for data in results:
                    if task_info.cancel_event.is_set():
                        break
                    try:
                        ...
                        logger.warning("Full update failed for %s: %s", data.get("gc_code", "?"), exc)
                    completed += 1
                    task_info.completed = completed
            except Exception as exc:
                logger.error("GC full update batch failed: %s", exc)
                completed += len(batch)
                task_info.completed = completed
            if i + BATCH < len(gc_codes) and not task_info.cancel_event.is_set():
                time.sleep(PAUSE)
    ...
    if oc_codes and not task_info.cancel_event.is_set():
        ...
        for platform, codes in platform_codes.items():
            ...
            user_id = acct.user_id if acct else ""
            client = OCClient(platform=platform, user_id=user_id)
            oc_done = completed - len(gc_codes)
            for i in range(0, len(codes), BATCH):
                if task_info.cancel_event.is_set():
                    break
                batch = codes[i:i + BATCH]
                task_info.phase = f"OC full update ({platform}): {oc_done + 1}–{min(oc_done + len(batch), len(oc_codes))} of {len(oc_codes)}"
                try:
                    results = client.get_caches(batch, SyncMode.FULL)
                    for data in results:
                        if task_info.cancel_event.is_set():
                        ...
                        try:
                            ...
                            logger.warning("Full update failed for %s: %s", data.get("oc_code", "?"), exc)
                        completed += 1
                        oc_done += 1
                        task_info.completed = completed
                except Exception as exc:
                    logger.error("OC full update batch failed: %s", exc)
                    completed += len(batch)
                    oc_done += len(batch)
                    task_info.completed = completed
                if i + BATCH < len(codes) and not task_info.cancel_event.is_set():
                    time.sleep(PAUSE)
    ...
    if skipped_al:
        done_phase += f", {skipped_al} AL skipped"
    logger.info("Full update done: %s", done_phase)
    task_info.phase = done_phase
    return {"summary": summary, "updated": updated, "skipped_al": skipped_al}
...
def _oc_link_refresh(qs, task_info):
    ...
    seen = set()
    oc_pairs = []
    for oc_code, _pk in qs.exclude(oc_code="").values_list("oc_code", "pk"):
    ...
    try:
        ...
        platform_codes = group_by_platform(oc_pairs)
...
def _gc_recent_logs(qs, task_info):
    ...
    for i, code in enumerate(gc_codes):
        if task_info.cancel_event.is_set():
            break
        try:
            saved, _ = gc_access.fetch_recent_gc_logs(code, count=50)
            total_new += saved
        except Exception as exc:
...
def _gc_all_logs(qs, task_info):
    ...
    for i, code in enumerate(gc_codes):
        if task_info.cancel_event.is_set():
            break
        try:
            saved = gc_access.fetch_all_gc_logs(code)
            total_new += saved
        except Exception as exc:
...
def _verify_ftf(qs, task_info):
    ...
    if gc_candidates:
        ...
        for code in gc_candidates:
            if task_info.cancel_event.is_set():
                break
            try:
                saved, _ = gc_access.fetch_recent_gc_logs(code, count=10)
                total_new += saved
            except Exception as exc:
    ...
    if oc_candidates and not task_info.cancel_event.is_set():
        ...
        platform_codes = group_by_platform(oc_candidates)
...
def _alc_only(qs, task_info):
    """Refresh all Adventure Lab caches in the queryset via the AL API.

    Adventures with a GUID are fetched directly.  Adventures without a GUID
    trigger a 10 m location search to find and attach the correct GUID first.
    """
    from geocaches.models import Adventure
    from geocaches.sync.service import sync_al_by_guids, sync_al_no_guid_adventure
    from geocaches.sync.gc_access import get_al_client

    al_qs = qs.filter(al_code__gt="")
    all_advs = Adventure.objects.filter(stages__in=al_qs).distinct()

    guids = list(
        all_advs.exclude(adventure_guid="")
        .values_list("adventure_guid", flat=True)
    )
    no_guid_advs = list(all_advs.filter(adventure_guid=""))

    total = len(guids) + len(no_guid_advs)
    task_info.total = total
    task_info.phase = (
        f"Refreshing {len(guids)} AL adventure(s)"
        + (f" + {len(no_guid_advs)} without GUID" if no_guid_advs else "")
    )
    logger.info(
        "ALC-only update: %d with GUID, %d without GUID", len(guids), len(no_guid_advs)
    )

    if not total:
        task_info.phase = "Done: no Adventure Lab caches"
        return {"updated": 0}

    created = updated = failed = 0

    # --- GUID path ---
    if guids and not task_info.cancel_event.is_set():
        try:
            stats = sync_al_by_guids(
                guids,
            ...
            logger.error("ALC-only (GUID) update failed: %s", exc)
            failed += len(guids)

    # --- No-GUID path (location search + match) ---
    if no_guid_advs and not task_info.cancel_event.is_set():
        client = get_al_client()
        # sync_al_by_guids overwrites task_info.total — restore combined total.
        task_info.total = total
        task_info.completed = len(guids)
        completed_offset = len(guids)
        task_info.phase = f"Searching by location for {len(no_guid_advs)} unmatched ALC(s)"
        for i, adv in enumerate(no_guid_advs):
            if task_info.cancel_event.is_set():
                break
            try:
                ...
                logger.error("No-GUID AL sync failed for %s: %s", adv.code, exc)
                failed += 1
            task_info.completed = completed_offset + i + 1

    done_phase = (
        f"Done: {created} created, {updated} updated"
        + (f", {failed} failed" if failed else "")
    )
    logger.info("ALC-only update done: %s", done_phase)
    task_info.phase = done_phase
    return {"updated": updated, "created": created, "failed": failed}


def _light_update_alc(qs, task_info):
    """Light-update GC/OC caches, then refresh all AL caches in the queryset."""
    # Phase 1: light update for GC + OC
    _light_update(qs, task_info)
    if task_info.cancel_event.is_set():
        return {"cancelled": True}

    # Phase 2: ALC refresh
    _alc_only(qs, task_info)
    return {}


def refresh_single_alc(adventure_id, *, task_info=None):
    """Refresh one Adventure Lab by GUID, then backfill per-stage found dates.

    Used by the cache-detail Refresh button so the task dock can show progress
    (the labs log fetch needs a GC website login and takes a few seconds).
    Reuses sync_al_by_guids(), which performs the refresh + stage-date backfill.
    """
    from geocaches.models import Adventure
    from geocaches.sync.service import sync_al_by_guids

    adv = Adventure.objects.filter(id=adventure_id).first()
    if adv is None or not adv.adventure_guid:
        return {"updated": 0, "errors": ["adventure missing or has no GUID"]}

    res = sync_al_by_guids([adv.adventure_guid], task_info=task_info)
    return {
        "created": res.created,
        "updated": res.updated,
        "failed": res.failed,
