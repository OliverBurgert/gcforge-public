...
def start_deletion(pk_list: list[int]) -> bool:
    ...
    AL parents in the selection drag their stages along (the parent→stage link
    runs through the shared Adventure record, not a cascading FK).
    """
    from geocaches.services.trash import stage_pks_for_parents

    extra = stage_pks_for_parents(pk_list)
    if extra:
        pk_list = list(pk_list) + extra
    ...
    def _run():
        try:
            ...
            for i in range(0, len(pk_list), BATCH):
                ...
                with db_write():
                    Geocache.objects.filter(pk__in=batch).update(deleted_at=deleted_at)
                with _lock:
