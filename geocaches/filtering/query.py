...
from geocaches.filtering.filters import FILTER_CHAIN
...
def apply_filters(qs: QuerySet, params: dict) -> QuerySet:
    ...
    :mod:`geocaches.filtering.filters` for the per-param rationale.
    """
...
def apply_filter_expr(qs: QuerySet, params: dict) -> tuple:
    ...
    (see :mod:`geocaches.filtering.filters`), not as a deprecated codepath.
    """
    from geocaches.filtering.filter_expr import (
        FilterExprError,
        Group,
...
def apply_where_clause(qs: QuerySet, params: dict) -> tuple:
    ...
    from geocaches.models import Geocache, SavedWhereClause
...
def annotate_distance(qs: QuerySet, ref) -> QuerySet:
    ...
    Uses the DistanceCache table when populated (fast indexed join) and falls
    back to the Python-based SQLite haversine callback for rows that have no
    cached entry yet — the cache is filled in the background
    (``geo.distance_cache.ensure_cached``), so a request may legitimately meet a
    partially filled one and must still see every cache at its true distance.
    SQLite's COALESCE short-circuits, so the fallback costs nothing on the rows
    that are cached.
    """
    ...
    if not has_cache:
        return qs.annotate(distance_km=raw_distance, bearing_deg=raw_bearing)
    ...
    return qs.annotate(
        ...
        distance_km=Coalesce(F("_dc__distance_km"), raw_distance, output_field=FloatField()),
        bearing_deg=Coalesce(F("_dc__bearing_deg"), raw_bearing, output_field=FloatField()),
    )
...
def _match_saved_area(geo_value: str):
    ...
    from geocaches.models import SavedAreaFilter
...
def apply_all(qs: QuerySet, params: dict, ref=None,
    ...
    # don't leak.
    from geocaches.filtering import filter_expr as _fx
    ...
    if params.get("flag") == "alc_loggable_at_center":
        from geocaches.filtering.filters import apply_alc_loggable_filter
