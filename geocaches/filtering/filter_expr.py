...
from geocaches.models import FOUND_LOG_TYPES as _FOUND_LOG_TYPES
...
def _register_date(field_name: str) -> None:
    ...
    # Day-of-month / month-of-year, matched across all years (e.g. "found on
    # the 21st of any month" or "found in any December"). Out-of-range values
    # (day > 31, month > 12) are dropped rather than raising, since the UI's
    # <input min/max> should already prevent them.
    def day_month(v):
        ...
        def _bounded(x, lo, hi):
            try:
                ...
                return None
            return xi if lo <= xi <= hi else None

        day = _bounded(v.get("day"), 1, 31)
        month = _bounded(v.get("month"), 1, 12)
        if day is None and month is None:
            return Q()
        q = Q()
        if day is not None:
            q &= Q(**{f"{field_name}__day": day})
        if month is not None:
            q &= Q(**{f"{field_name}__month": month})
        return q
...
# Adventure Lab stage find confidence. Bespoke (not _register_enum) because
# "" (unknown) is a real, selectable member here — _register_enum's "in"/
# "not_in" go through _normalise_list(), which drops empty strings, so a
# combined "Unknown + App-verified" selection would silently lose "Unknown".
def _found_accuracy_q(v, *, negate: bool) -> Q:
    items = v if isinstance(v, (list, tuple)) else ([v] if v is not None else [])
    if not items:
        return Q()
    q = Q(found_accuracy__in=items)
    return ~q if negate else q


_REGISTRY[("found_accuracy", "in")] = lambda v: _found_accuracy_q(v, negate=False)
_REGISTRY[("found_accuracy", "not_in")] = lambda v: _found_accuracy_q(v, negate=True)
_REGISTRY[("found_accuracy", "is_none")] = lambda _v: Q(found_accuracy="")
...
# Elevation's "is_null" needs its own override: a cache with a user-supplied
# elevation_user but no auto elevation (typed in manually, or set via the map's
# "Update elevation from here") has real elevation data — it must not show up
# in a "missing elevation" filter. Mirrors the legacy ``?elevation=none``
# bucket in filters.py, which the generic range registration above can't
# express (it only sees the bare ``elevation`` field).
def _elevation_is_null(v) -> Q:
    missing = Q(elevation__isnull=True, elevation_user__isnull=True)
    return missing if v else ~missing


_REGISTRY[("elevation", "is_null")] = _elevation_is_null
...
def _bearing_direction_in(v):
    ...
    if not dirs:
        return Q()
    from geocaches.filtering.query import BEARING_RANGES
    q = Q()
    for d in dirs:
...
def _alc_geofence_contains_point(v):
    ...
    from geocaches.geo import haversine_km as _haversine_km
    from geocaches.models import Geocache
...
def _ftf_possible_is_true(_v):
    from geocaches.models import EVENT_CACHE_TYPES, Geocache
...
def _logs_last_log_type_in(v):
    ...
    from geocaches.models import Geocache, Log
...
def condition_to_label(field: str, op: str, value) -> str:
    ...
    if field == "found_accuracy":
        # Values are internal codes (app_verified, …), not display labels —
        # resolve through the choices so the chip reads like the UI does.
        # NB: don't use _normalise_list() here — it drops "" (Unknown).
        from geocaches.models import FoundAccuracy
        names = dict(FoundAccuracy.choices)

        def _fa_label(v):
            return str(names.get(v, v)) if v else "Unknown"

        items = value if isinstance(value, (list, tuple)) else [value]
        if op == "in":
            return f"Found accuracy: {', '.join(_fa_label(v) for v in items)}"
        if op == "not_in":
            return f"Found accuracy ∉: {', '.join(_fa_label(v) for v in items)}"
        if op == "is_none":
            return "Found accuracy: Unknown"
    ...
    if field in _DATE_FIELDS:
        ...
        if op == "day_month":
            ...
            if day and month_name:
                return f"{label}: {day}. {month_name} (all years)"
            if day:
                return f"{label}: day {day} (all years)"
            if month_name:
                return f"{label}: {month_name} (all years)"
