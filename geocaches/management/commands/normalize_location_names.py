...
def _fetch_for_coords(lat: float, lon: float, iso_country_code: str) -> dict[str, str]:
    ...
    if time.time() < _enr._nominatim_blocked_until:
        return {}
    try:
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--country",
            default="",
            help="Limit to one ISO 3166-1 alpha-2 country code (e.g. DE). "
                 "Recommended — granularity conventions differ per country.",
        )
    ...
    def handle(self, *args, **options):
        ...
        if check_state:
            self._process_field("state", dry_run, prefix, country)
        if check_county:
            self._process_field("county", dry_run, prefix, country)

    def _process_field(self, field_name: str, dry_run: bool, prefix: str,
                       country: str = "") -> None:
        import time
        ...
        with transaction.atomic():
            for iso_code, old_value, new_value, _count in replacements:
