...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for cache in candidates:
            ...
            if not prefix:
                continue
            # Live oc_codes are unique — a cache that already owns this code
            # has to be merged by hand (Tools -> Duplicate caches).
            owner = live_code_conflict("oc_code", cache.gc_code, exclude_pk=cache.pk)
            if owner is not None:
                self.stdout.write(self.style.WARNING(
                    f"  SKIP {cache.gc_code}: cache #{owner.pk} already uses that oc_code — "
                    f"merge them first."
                ))
                continue
