...
from geocaches.importers.lookups import cache_type_to_gpx, WAYPOINT_TYPE_TO_PREFIX, VALID_WP_PREFIXES
from geocaches.models import EVENT_CACHE_TYPES
...
def _build_note_entries(cache, cc, gc_username, opts):
    ...
    # Iterate the prefetched manager once and partition in Python — any
    # .filter()/.order_by() on cache.notes would bypass the prefetch cache
    # and trigger N+1 queries on bulk exports.
    want_notes = bool(opts.get("notes_gcforge"))
    want_field_notes = bool(opts.get("notes_field_notes"))
    if want_notes or want_field_notes:
        all_notes = sorted(
            cache.notes.all(),
            # created_at is tz-aware (USE_TZ=True); the sentinel must be too,
            # or sorting raises when a note has no created_at.
            key=lambda n: n.created_at or _dt.min.replace(tzinfo=_tz.utc),
        )
    else:
        all_notes = []
    ...
    if want_field_notes:
        for note in all_notes:
            if note.note_type != "field_note":
                continue
            dt = note.created_at.strftime("%Y-%m-%d") if note.created_at else "unknown date"
            entries.append(f"Field note from {dt}:\n{note.body or ''}")

    if opts.get("notes_corrected") and cc:
        from geocaches.geo.coords import format_dd, format_dmm, format_dms
        ts = cc.updated_at.strftime("%Y-%m-%d %H:%M") if cc.updated_at else "unknown"
        dd_lat, dd_lon = format_dd(cc.latitude, cc.longitude)
        dmm_lat, dmm_lon = format_dmm(cc.latitude, cc.longitude)
        dms_lat, dms_lon = format_dms(cc.latitude, cc.longitude)
        entries.append(
            f"Corrected coordinates:\n"
            f"DD:  {dd_lat}, {dd_lon}\n"
            f"DMM: {dmm_lat}, {dmm_lon}\n"
            f"DMS: {dms_lat}, {dms_lon}\n"
            f"Note: {cc.note or '(none)'}\n"
            f"Updated: {ts}"
        )

    al_detail = getattr(cache, "al_detail", None)
    if al_detail and al_detail.user_answer:
        status = ""
        if al_detail.answer_is_correct is True:
            status = " [✓ verified]"
        elif al_detail.answer_is_correct is False:
            status = " [✗ incorrect]"
        entries.append(f"ALC Answer{status}:\n{al_detail.user_answer}")
...
def export_gpx(queryset, gc_username: str = "", opts: dict | None = None) -> bytes:
    ...
    qs = queryset.select_related("corrected_coordinates", "adventure", "al_detail").prefetch_related(
    ...
    for cache in all_caches:
        al_detail = getattr(cache, "al_detail", None)
        if cache.adventure_id and al_detail is not None:
    ...
    for cache in all_caches:
        ...
        al_detail = getattr(cache, "al_detail", None)
        is_alc_stage = is_alc and al_detail is not None
        is_alc_parent = cache.is_al_parent
        ...
        al_q = getattr(al_detail, "question_text", None) if al_detail else None
        long_desc = (cache.long_description or "") + (f"\n\nQuestion: {al_q}" if al_q else "")
        _gs(gs_cache, "long_description", long_desc, html="True")
        _gs(gs_cache, "encoded_hints", cache.hint)

        # --- Logs ---
        logs_el = SubElement(gs_cache, f"{{{GS_NS}}}logs")

        # Use the prefetched manager and slice in Python so we don't re-query.
        all_logs = list(cache.logs.all())
        if logs_cap is not None:
            all_logs = all_logs[:logs_cap]
        ...
        # --- Child waypoints ---
        # Filter in Python on the prefetched list to keep this O(1) queries
        # per cache regardless of options chosen.
        include_hidden = opts.get("wp_hidden", False)
        include_completed = opts.get("wp_completed", True)
        wp_filter = [
            wp for wp in cache.waypoints.all()
            if (include_hidden or not wp.is_hidden)
            and (include_completed or not wp.is_completed)
        ]
        ...
        for wp in wp_filter:
            ...
            wp_prefix = (wp.prefix if wp.prefix in VALID_WP_PREFIXES else None) or WAYPOINT_TYPE_TO_PREFIX.get(wp.waypoint_type, "WP")
            _gpx(wp_el, "name", f"{wp_prefix}{code[2:]}")
            _gpx(wp_el, "desc", wp.name or wp.waypoint_type)
            if wp.note:
                _gpx(wp_el, "cmt", wp.note)
            sym = _WAYPOINT_SYM.get(wp.waypoint_type, "Reference Point")
            _gpx(wp_el, "sym",  sym)
            _gpx(wp_el, "type", f"Waypoint|{sym}")
        ...
        if is_alc_parent and alc_stages_mode in ("child_only", "child_and_export"):
            stages = sorted(adv_to_stages.get(cache.adventure_id, []),
                            key=lambda s: getattr(getattr(s, "al_detail", None), "stage_number", None) or 0)
            for stage in stages:
                alc_completed_mode = opts.get("alc_completed", "found_invisible")
                ...
                if stage.latitude is None or stage.longitude is None:
                    continue
                sn = getattr(stage.al_detail, "stage_number", None) if getattr(stage, "al_detail", None) else None
                stage_sym = "Stages of a Multicache"
                stage_el = SubElement(gpx, f"{{{GPX_NS}}}wpt", {
                    "lat": f"{stage.latitude:.6f}",
                    "lon": f"{stage.longitude:.6f}",
                })
                stage_code = stage.al_code or f"{code}-{sn}"
                _gpx(stage_el, "name", f"S{stage_code[-2:].lstrip('0') or sn}{code[2:]}")
                stage_al_detail = getattr(stage, "al_detail", None)
                stage_q = getattr(stage_al_detail, "question_text", None) if stage_al_detail else None
                stage_desc = (stage.name or f"Stage {sn}") + (f"\n\nQuestion: {stage_q}" if stage_q else "")
                _gpx(stage_el, "desc", stage_desc)
