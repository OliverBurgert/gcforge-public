...
def _fieldnotes_dir() -> Path:
    ...
    return Path(settings.FIELDNOTES_DIR)


def download_gc_fieldnotes() -> Path:
    """Fetch log drafts via the active GC backend and save as a field note file.
    ...
    on any failure), or always uses the website when preferred.
    """
    from geocaches.sync import gc_access

    if gc_access.gc_access_mode() == "web_preferred":
        from geocaches.sync.gc_web.fieldnotes import download_gc_fieldnotes_web
        return download_gc_fieldnotes_web()

    from geocaches.feature_flags import gc_api_available
    if gc_api_available():
        try:
            return _download_gc_fieldnotes_api()
        except Exception as exc:  # noqa: BLE001
            logger.warning("GC field-note draft download via official API failed, falling back to web: %s", exc)

    from geocaches.sync.gc_web.fieldnotes import download_gc_fieldnotes_web
    return download_gc_fieldnotes_web()


def _download_gc_fieldnotes_api() -> Path:
    """Fetch log drafts from the official GC API and save as a field note file.
