...
from geocaches.importers.gpx_common import parse_gpx_file
...
from geocaches.services.save_alc import (
    _get_or_create_adventure, _upsert_parent_geocache, save_alc_stage,
)
...
def _import_format_a(root, tags, stats: ImportStats, now) -> None:
    ...
    for wpt in root.findall(gpx("wpt")):
        ...
        try:
            save_alc_stage(gc_code, model_fields, tags, stats)
...
def _import_format_b(root, tags, stats: ImportStats, now) -> None:
    ...
    for adv_lc_code, stage_list in stages_by_parent.items():
        ...
        for s_code, stage_wpt in stage_list:
            ...
            try:
                save_alc_stage(canonical_code, model_fields, tags, stats)
            except Exception as exc:
