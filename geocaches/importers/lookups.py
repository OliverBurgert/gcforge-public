...
SYM_TO_WAYPOINT_TYPE: dict[str, str] = {
    ...
    "Virtual Stage":            WaypointType.QUESTION,
...
# ---------------------------------------------------------------------------
# Waypoint code prefixes (GSAK / GC.com convention)
# ---------------------------------------------------------------------------

WAYPOINT_TYPE_TO_PREFIX: dict[str, str] = {
    "Parking":   "PK",
    "Stage":     "ST",
    "Question":  "QA",
    "Final":     "FL",
...
# Prefixes that carry type information (anything else is a numeric/arbitrary prefix)
VALID_WP_PREFIXES: frozenset[str] = frozenset(WAYPOINT_TYPE_TO_PREFIX.values())
...
def parse_gpx_time(s: str) -> Optional[time]:
    """Extract the time component from a GPX ISO datetime string.

    Returns None when absent, malformed, or midnight (00:00:00), since
    midnight almost always means "date only, no time specified".
    """
