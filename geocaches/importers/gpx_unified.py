...
def import_gpx(
    ...
    for wpt_el in root.findall(f"{{{NS_GPX}}}wpt"):
        ...
        if prefix == "GC":
            _import_gc_wpt(wpt_el, cache_el, code, gc_wpts, tags, now, stats, attribute_cache)
        elif prefix in OC_PREFIXES:
            _import_oc_wpt(wpt_el, cache_el, code, oc_wpts, tags, now, stats, attribute_cache)
        # LC (Adventure Lab) and unknown prefixes are silently skipped

    return stats


def _save_with_retry(save_call):
    """Run *save_call* under the write lock; retry on transient SQLite locks.

    Mirrors the retry loop in ``enrichment.enrich_geocache``: SQLite occasionally
    raises "database is locked" even with a 30 s busy_timeout when another
    connection is writing.  The lock makes that rare; the retry handles the
    edge cases (Django admin, management commands, etc.).
    """
...
def _import_gc_wpt(wpt_el, cache_el, gc_code, wpts_by_parent, tags, now, stats, attribute_cache=None):
...
def _import_oc_wpt(wpt_el, cache_el, oc_code, wpts_by_parent, tags, now, stats, attribute_cache=None):
