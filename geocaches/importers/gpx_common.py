...
# Single source of truth: geocaches.models.EVENT_CACHE_TYPES. Note this is
# matched against the *normalized* CacheType (gpx_type_to_cache_type at line
# ~103), not the raw GPX <type> string — so the enum-based canonical set fits.
from geocaches.models import EVENT_CACHE_TYPES as _GPX_EVENT_TYPES
...
def parse_base_cache_fields(wpt_el: ET.Element, cache_el: ET.Element) -> dict:
    ...
    time_str = _text(wpt_el, gpx("time"))
    hidden_date = parse_gpx_date(time_str)
    ...
    result = {
        ...
        "event_start_time": parse_gpx_time(time_str) if cache_type in _GPX_EVENT_TYPES else None,
        "event_end_time": None,
...
def build_attribute_cache() -> dict:
    """Load all Attribute rows once for fast (source, id, is_positive) lookup.

    Returned dict is mutable so save_geocache can insert newly-created rows
    on the fly. Callers reuse it across all caches in a single import to
    avoid one get_or_create per attribute occurrence.
    """
    from geocaches.models import Attribute
    return {
        (a.source, a.attribute_id, a.is_positive): a
...
def prepare_save_fields(
    ...
    *,
    ...
    for attr_id, attr_name, is_positive in attrs_data:
        # OC GPX uses Groundspeak-equivalent / OC-internal attribute ids;
        # canonicalise them onto OKAPI A-codes so GPX- and OKAPI-sourced OC
        # attributes share one key space (and icons, keyed by A-code, line
        # up).  GC imports keep their native ids untouched.
        if attr_source == "oc":
            from geocaches.oc_attribute_map import resolve_oc_attribute
            resolved = resolve_oc_attribute(attr_id, is_positive)
            if resolved is not None:
                attr_id, attr_name = resolved
        attr_dicts.append({
            "source": source_enum,
...
# XML 1.0's Char production only allows tab/LF/CR among C0 control codes —
# everything else in \x00-\x1F is illegal. A single such character anywhere
# in an otherwise-well-formed multi-thousand-line GPX file (they occasionally
# leak into GC/OC log text or descriptions from copy-pasted content) makes
# the *entire* document unparseable: expat's "reference to invalid character
# number: line X, column Y" error, all-or-nothing.
#
# Two distinct representations were both confirmed live 2026-08-15 against a
# real PQ ("Ueberlingen 30km bis 2016-04-01") that failed to import:
#
# 1. A literal control byte embedded directly in the text. Byte values
#    0x00-0x1F are encoded identically (one byte, same value) in UTF-8 as in
#    ASCII, so stripping them on raw bytes — before any encoding-aware
#    decoding — is safe regardless of the file's actual encoding.
# 2. GC's own PQ generator escaping a control character a finder typed into
#    a log as a numeric XML character reference, e.g. literal ASCII text
#    "&#x1E;" (RECORD SEPARATOR) — six printable, otherwise-legal bytes, so
#    (1)'s raw-byte strip alone doesn't touch it. The *reference* itself is
#    what expat rejects (a syntactically valid char-ref pointing at a
#    codepoint XML 1.0 doesn't allow), not any byte in its own encoding —
#    caught separately below.
_ILLEGAL_XML_BYTES_RE = re.compile(rb"[\x00-\x08\x0B\x0C\x0E-\x1F]")
_XML_CHAR_REF_RE = re.compile(rb"&#(x[0-9A-Fa-f]+|[0-9]+);")
...
def _drop_illegal_char_ref(match: "re.Match[bytes]") -> bytes:
    raw = match.group(1)
    codepoint = int(raw[1:], 16) if raw[:1] in (b"x", b"X") else int(raw)
    return b"" if not _is_legal_xml_char(codepoint) else match.group(0)


def strip_illegal_xml_bytes(data: bytes) -> bytes:
    """Remove XML-1.0-illegal control characters from raw GPX/XML content —
    both literal control bytes and numeric character references (``&#31;``,
    ``&#x1E;``, ...) that resolve to an illegal codepoint.

    Safe to call unconditionally — these have no legitimate meaning in
    cache/log text. See module comment above for why this operates on raw
    bytes rather than decoded text, and why both representations matter.
    """
    data = _ILLEGAL_XML_BYTES_RE.sub(b"", data)
    return _XML_CHAR_REF_RE.sub(_drop_illegal_char_ref, data)


def parse_gpx_bytes(data: bytes) -> ET.Element:
    """Parse GPX/XML content from bytes, stripping illegal XML control
    characters first. Returns the root Element."""
    return ET.fromstring(strip_illegal_xml_bytes(data))


def parse_gpx_file(path) -> ET.Element:
    """Parse a GPX/XML file from a path, stripping illegal XML control
    characters first. Returns the root Element — equivalent to
    ``ET.parse(path).getroot()`` but sanitized first."""
    return parse_gpx_bytes(Path(path).read_bytes())
...
def load_roots_from_zip(zip_path: Path) -> tuple[ET.Element, Optional[ET.Element]]:
    ...
    with zipfile.ZipFile(zip_path) as zf:
        ...
        main_root = parse_gpx_bytes(zf.read(main_name))
        wpts_root = parse_gpx_bytes(zf.read(wpts_name)) if wpts_name else None
