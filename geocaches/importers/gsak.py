...
from geocaches.services.save_alc import _get_or_create_adventure, _upsert_parent_geocache

# ---------------------------------------------------------------------------
# GSAK UserNote splitting
# ---------------------------------------------------------------------------

def _clean_admin(state: str, county: str, country: str) -> tuple[str, str]:
    """Normalise GSAK State/County the way Nominatim enrichment does, so
    GSAK-sourced names ("Stadt München", "Landkreis Esslingen") match the
    enrichment/boundary forms ("München", "Esslingen") and the database stays
    consistent across import sources."""
    iso = _country_to_iso(country)
    return (strip_admin_suffix(state, iso, "state"),
            strip_admin_suffix(county, iso, "county"))
...
def _save_alc_stage_format_a(
    ...
    state, county = _clean_admin(row["State"] or "", row["County"] or "", country)
    ...
    with db_write_atomic():
        ...
        if result.geocache and not result.locked:
            from geocaches.models import ALStageDetail
            detail, _ = ALStageDetail.objects.get_or_create(geocache=result.geocache)
            detail.stage_number = n
            if guid:
                detail.al_stage_uuid = guid
            if question_text:
                detail.question_text = question_text
            detail.save()
            # Promote found AFTER ALStageDetail exists so the guard doesn't reject it.
            if found_from_gsak and not result.geocache.found:
                result.geocache.found = True
                result.geocache.found_date = found_date_from_gsak
                result.geocache.save(update_fields=["found", "found_date"])
        ...
        try:
            with db_write_atomic():
                ...
                if result.geocache and not result.locked:
                    ...
                    detail.save()
                    # Promote found AFTER ALStageDetail exists so the guard doesn't reject it.
                    if found_from_parent and not result.geocache.found:
                        result.geocache.found = True
                        result.geocache.found_date = found_date_from_parent
                        result.geocache.save(update_fields=["found", "found_date"])
...
def _save_cache(
    ...
    gsak_state, gsak_county = _clean_admin(
        row["State"] or "", row["County"] or "", row["Country"] or "")
...
def import_gsak_ignore_list(db_path: Path) -> dict:
    """Read the Ignore table from a GSAK sqlite.db3 and insert as internal ignore entries.

    Skips codes that are already on the internal list (does not overwrite notes).
    Returns {'added': N, 'skipped_existing': M, 'db_name': str}.
    """
    ...
    added = skipped = 0
    note = f"gsak import from DB {db_label}"
    for icode, _iname in rows:
        code = (icode or "").strip().upper()
        if not code:
            continue
        _, created = IgnoreListEntry.objects.get_or_create(
            source=IgnoreSource.GSAK,
            oc_platform="",
            code=code,
            defaults={"notes": note},
        )
    ...
    return {"added": added, "skipped_existing": skipped, "db_name": db_label}
