...
__all__ = [
    ...
    "parse_cache_fields", "parse_trackable_mentions", "save_geocache", "import_gc_gpx",
...
def parse_trackable_mentions(cache_el: ET.Element) -> list[dict]:
    """Parse <groundspeak:travelbugs> into a list of mention dicts.

    Returns a list of {"gc_id": int|None, "ref_code": str, "name": str}.
    Empty list if the element is absent or empty.
    """
    out: list[dict] = []
    tb_el = cache_el.find(gs("travelbugs"))
    if tb_el is None:
        return out
    for tb in tb_el.findall(gs("travelbug")):
        ref = (tb.get("ref") or "").strip()
        if not ref:
            continue
        gc_id = None
        gc_id_str = tb.get("id")
        if gc_id_str:
            try:
                ...
                pass
        name = _text(tb, gs("name"))
        out.append({"gc_id": gc_id, "ref_code": ref, "name": name})
    return out
...
def _parse_wpts_root(root: ET.Element) -> dict[str, list[dict]]:
    ...
    for wpt_el in root.findall(f"{{{NS_GPX}}}wpt"):
        ...
        wpt_data = {
            "lookup": lookup,
            "prefix": lookup[:2].upper() if lookup[:2].upper() in VALID_WP_PREFIXES else "",
...
def parse_wpts_gpx(path: str) -> dict[str, list[dict]]:
    ...
    """
    root = parse_gpx_file(path)
    return _parse_wpts_root(root)
...
def save_geocache(
    ...
    tb_mentions: Optional[list[dict]] = None,
    ...
    result = _save(
        ...
        trackable_mentions=tb_mentions,
        update_source="gc",
...
def import_gc_gpx(
    ...
    from .gpx_common import build_attribute_cache
            ...
            for attempt in range(6):
                try:
                    with db_write_atomic():
                        ...
                        tb_mentions = parse_trackable_mentions(cache_el)
