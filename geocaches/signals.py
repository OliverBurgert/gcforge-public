...
@receiver(pre_save, sender="preferences.ReferencePoint")
def snapshot_ref_point_coords(sender, instance, **kwargs):
    """Record whether this save moves the point — post_save can no longer tell."""
    if instance.pk is None:
        instance._gcf_coords_moved = False
        return
    old = (
        sender.objects.filter(pk=instance.pk)
        .values_list("latitude", "longitude").first()
    )
    instance._gcf_coords_moved = (
        old is not None and old != (instance.latitude, instance.longitude)
    )


@receiver(post_save, sender="preferences.ReferencePoint")
def invalidate_distance_cache_on_ref_save(sender, instance, created, **kwargs):
    """Drop the cached distances when a reference point actually moves.

    Renaming a point, or making it the default, must keep its rows — a rebuild
    costs seconds (geocaches/geo/distance_cache.py).  A freshly created point
    has no rows yet; the next request schedules the background fill.
    """
    if created or not getattr(instance, "_gcf_coords_moved", True):
        return
...
def cascade_al_parent_to_stages(sender, instance, **kwargs):
    ...
    Deliberately not filtered by ``deleted_at`` — this cascade covers two
    distinct legitimate callers: purging an already-trashed parent (whose
    stages were soft-deleted alongside it, see services.trash) and hard-
    deleting a *live* parent (e.g. ``merge_duplicate_adventure``, which
    relies on this to also remove the live stage siblings it's discarding).
    """
