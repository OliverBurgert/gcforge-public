...
from .enums import CacheSize, CacheStatus, CacheType, FoundAccuracy
...
class Geocache(models.Model):
    ...
    # Confidence in found_date/location for an Adventure Lab stage find; blank
    # = unknown/not assessed. Never inferred automatically — see FoundAccuracy.
    found_accuracy = models.CharField(max_length=25, choices=FoundAccuracy, blank=True, default="")
    ...
    # True on the synthetic parent row standing for a whole adventure; stages
    # carry an ALStageDetail instead.  Written by services.save_alc's
    # _upsert_parent_geocache (the only parent creator) and read by
    # services.adventures.is_al_parent.
    is_al_parent = models.BooleanField(default=False, db_index=True)
    ...
    class Meta:
        constraints = [
            ...
            # An external code identifies at most one cache that is not in the
            # Trash.  Soft-deleted rows keep their codes so a re-import can
            # build a fresh record beside them (see geocaches.services.codes).
            models.UniqueConstraint(
                fields=["gc_code"],
                condition=models.Q(deleted_at__isnull=True) & ~models.Q(gc_code=""),
                name="uniq_live_gc_code",
            ),
            models.UniqueConstraint(
                fields=["oc_code"],
                condition=models.Q(deleted_at__isnull=True) & ~models.Q(oc_code=""),
                name="uniq_live_oc_code",
            ),
            models.UniqueConstraint(
                fields=["al_code"],
                condition=models.Q(deleted_at__isnull=True) & ~models.Q(al_code=""),
                name="uniq_live_al_code",
            ),
        ]
        indexes = [
