...
class FoundAccuracy(models.TextChoices):
    """Confidence in the location/datetime of an Adventure Lab stage find.

    Lives on ``Geocache.found_accuracy`` (per stage, same granularity as
    found/found_date). Blank (not a member here) means "unknown / not
    assessed" — never inferred automatically from the log source, since even
    an app-submitted log isn't proof of presence (spoofable GPS, and stages
    with no geofencing radius accept an answer from anywhere).
    """
    APP_VERIFIED = "app_verified", _("App-verified")
    VISITED_SAME_DAY = "visited_same_day", _("Visited — logged same day")
    VISITED_DIFFERENT_DAY = "visited_different_day", _("Visited — logged different day")
    ARMCHAIR = "armchair", _("Armchair (not visited)")
