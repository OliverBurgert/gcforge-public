        ...
        for data in normalized:
            ...
            try:
                ...
                if (client.platform == "gc" and mode == SyncMode.FULL
                        and data.get("found")):
                    try:
                        from . import gc_access
                        gc_access.ensure_my_gc_logs(data.get("gc_code", ""))
...
def _backfill_al_stage_dates(guids: list[str], task_info=None) -> int:
    ...
    try:
        ...
        updated = get_labs_client().sync_stage_dates_for_adventures(gc_account.user_id, guids)
        if updated:
...
def sync_al_by_guids(
    ...
    ``found_accuracy`` (see ``geocaches.models.FoundAccuracy``) is stamped on
    stages this sync newly marks found; pass "" to leave it unknown.
    """
    from geocaches.models import Tag
    ...
    for i, guid in enumerate(guids):
        ...
        try:
            ...
            _, stats = save_adventure_from_api(data, tags=tag_objs, found_accuracy=found_accuracy)
...
def sync_al_no_guid_adventure(adv, client) -> "object | None":
    ...
    if lat is None or lon is None:
        parent = adv.stages.filter(is_al_parent=True).first()
        if parent:
...
def import_al_founds_from_logs(
    ...
    from geocaches.services.save_alc import save_adventure_from_api
    from geocaches.sync.gc_access import get_al_client, get_labs_client
    ...
    try:
        grouping = get_labs_client().fetch_found_grouping(account_guid)
    except Exception as exc:
...
def refresh_membership_level() -> int:
    """
    Fetch the GC user's profile to check the membership level.
    ...
    level = get_gc_client().get_my_profile()["membership_level"]
