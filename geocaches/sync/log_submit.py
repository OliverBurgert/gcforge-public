...
# Auto-tag applied to a TB after a standalone action (see
# submit_standalone_tb_action()) — same vocabulary as the trackables list
# view's "Sync discovered"/"Sync moved" scopes
# (geocaches.views.trackable_list._SCOPE_TAGS), so bulk-logged TBs show up
# under the same tag filter as ones pulled in via that sync.
_TB_ACTION_TAG: dict[str, str] = {
    "discover": "discovered",
    "retrieve": "moved",
}
...
def submit_standalone_tb_action(action: str, tracking_code: str, text: str = "") -> "TrackableLogSubmitResult":
    """Submit a Discover/Retrieve trackable log with no cache context.

    Used by the trackables list view's bulk "log by tracking code" button —
    unlike ``_submit_single_tb_action`` (always submitted alongside a specific
    cache's log), this stands alone: no ``geocacheReferenceCode``, no
    drop/visit/grab_chain support, no image attachments. The caller only
    knows the private tracking code, never the public ``TB#######`` — that's
    resolved here via ``verify_tracking_code()`` first.
    """
    ...
    tb_result = TrackableLogSubmitResult(ref_code="", action=action)
    ...
    try:
        meta = client.verify_tracking_code(tracking_code)
    ...
    try:
        resp = client.submit_trackable_log(
            ref, log_type, iso_str, text, tracking_code=tracking_code,
        )
        tb_result.source_id = resp.get("referenceCode", "")
        tb_result.success = True
        logger.info("Submitted standalone TB log %s on %s (%s)", tb_result.source_id, ref, action)
    except Exception as exc:  # noqa: BLE001
        tb_result.error = str(exc)
        logger.warning("Standalone TB log submission failed for %s (%s): %s", ref, action, exc)
        return tb_result
    ...
    try:
        ...
        trackable, _ = Trackable.objects.get_or_create(
            reference_code=ref, defaults={"name": meta.get("name") or ref},
        )
        TrackableLog.objects.create(
            trackable=trackable, log_type=log_type, logged_date=logged_at.date(),
            logged_at=logged_at, text=text, user_name=user_name,
            source_id=tb_result.source_id, is_local=True,
        )
        tag_name = _TB_ACTION_TAG.get(action)
        if tag_name:
            from geocaches.models import Tag
            tag, _ = Tag.objects.get_or_create(name=tag_name)
            trackable.tags.add(tag)
    except Exception as exc:  # noqa: BLE001
        logger.warning("Failed to persist local TB log for %s: %s", ref, exc)

    try:
        _update_local_tb_state(ref, action, None)
    except Exception as exc:  # noqa: BLE001
        logger.warning("Failed to update local TB state for %s (%s): %s", ref, action, exc)

    # Same opt-in auto-fetch as `_submit_single_tb_action` — pull full metadata
    # from GC and persist the typed tracking code locally if we didn't already
    # have one (see that function's own comment for why).
    try:
        from preferences.models import UserPreference
        if UserPreference.get("auto_fetch_tb_on_log", False):
            ...
            try:
                ...
                logger.warning("auto-fetch %s after standalone log submission failed: %s", ref, exc)
...
def submit_log(
    ...
    *,
    ...
    # --- GC submission ---
    gc_backend = None  # "gc_api" or "gc_web" — decides how images get attached below
    if "gc" in platforms and cache.gc_code:
        result.gc_success = False
        try:
            from geocaches.sync.gc_access import submit_gc_log
            gc_backend, resp = submit_gc_log(
            ...
            logger.info("Submitted GC log %s for %s via %s", result.gc_ref_code, cache.gc_code, gc_backend)
            if give_favourite:
    ...
    if images:
        ...
        if result.gc_success and result.gc_ref_code and gc_backend == "gc_web":
            # The web API can't attach images to an existing log directly —
            # each image must be uploaded first (getting a guid), then the
            # guids are attached via attach_images() (which re-sends the log's
            # own type/text/date). See log_client.attach_images()'s docstring.
            try:
                from geocaches.sync.gc_web.log_client import upload_log_image, attach_images
                guids = []
                for att in image_list:
                    try:
                        processed, mime = process_image(att)
                        upload_result = upload_log_image(
                            cache.gc_code, processed, mime,
                            name=att.title, description=att.description,
                        )
                        guid = upload_result.get("guid", "")
                        if guid:
                            guids.append(guid)
                        logger.info("GC web image uploaded for %s: %s", cache.gc_code, att.filename)
                    except Exception as exc:
                        msg = f"GC image '{att.filename}': {exc}"
                        result.image_errors.append(msg)
                        logger.warning("GC web image upload failed: %s", msg)
                if guids:
                    attach_images(
                        cache.gc_code, result.gc_ref_code, log_type, iso_str, to_gc(text), guids,
                    )
            except Exception as exc:
                result.image_errors.append(f"GC image attach failed: {exc}")
        elif result.gc_success and result.gc_ref_code:
            try:
    ...
    try:
        ...
        for raw in tb_actions:
            result.tb_results.append(TrackableLogSubmitResult(
                ...
                action=raw.get("action") or "",
                success=False,
                error=f"GC trackable client unavailable: {exc}",
            ))
        return
...
def _submit_single_tb_action(
    *, cache, logged_at: datetime, iso_str: str, raw: dict, action: str,
    ...
    if tb_images and tb_result.source_id:
        ...
        for img_res in upload_images_to_gc_trackable_log(client, tb_result.source_id, tb_images):
            if not img_res.ok:
