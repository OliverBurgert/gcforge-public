...
class BasePlatformClient(ABC):
    ...
    def search_lite_by_center(
        ...
        *,
        ...
        """Fast preview path: return normalized LIGHT cache dicts straight from
        the search endpoint (no separate detail fetch), distance-sorted and
        stopping once results pass radius_m.

        Returns None when the platform's search can't supply the lightweight
        fields — the caller then falls back to search_by_* + get_caches().
        """
        return None
    ...
    def search_lite_by_bbox(
        ...
        *,
        ...
        """Bounding-box variant of search_lite_by_center(). Returns None when
        unsupported (caller falls back to search_by_bbox + get_caches)."""
        return None
    ...
    def search_criteria(
        ...
        *,
        ...
        """Return codes matching a criteria dict (owner/type/D-T/size/…).

        Optional ``bbox`` (south, west, north, east) constrains geographically.
        Default: criteria search unsupported on this platform → empty list.
        """
        return []
    ...
    def search_criteria_lite(
        ...
        *,
        ...
        """Criteria search returning normalized LIGHT dicts (preview fast path),
        or None when unsupported (caller falls back to search_criteria + fetch)."""
        return None
...
@runtime_checkable
class GCLogClient(Protocol):
    """Shared shape for submitting a GC geocache log.

    Implemented structurally (no inheritance needed) by
    ``gcprivate.gc_client.GCClient`` (official partner API) and
    ``geocaches.sync.gc_web.log_client.GCWebLogClient`` (website/tRPC
    fallback). Picked at runtime by ``geocaches.sync.gc_access.submit_gc_log()``
    per the ``gc_access_mode`` preference — see
    ``docs/reference/geocaching-com-web.md`` §6.

    Deliberately narrow: **image attachment is not part of this Protocol.**
    The two backends attach images in a fundamentally different order — the
    official API creates the log first, then POSTs each image against the
    existing log's ref; the web/tRPC path uploads each image *first* and
    references the resulting guids when creating the log, so no image can be
    attached to an already-existing log without first fetching its current
    image list (not a call either backend implements today). Forcing a
    shared method signature here would either lie about the ordering or need
    functionality that doesn't exist yet — so each backend's image-upload
    method keeps its own natural shape instead of pretending to match.
    """
    ...
    def submit_log(
        ...
        *, use_favourite_point: bool = False,
        ...
        """Submit a geocache log. Returns a dict with a ``referenceCode`` key."""
        ...
