...
from ..geo.countries import name_to_iso
...
class OCClient(BasePlatformClient):
    ...
    def normalize(self, raw: dict, mode: SyncMode) -> dict:
        ...
        fields = {
            ...
            "primary_source": self.platform,
        }
        # OKAPI gives a display name, not an ISO code — derive it the same
        # way the GPX importer does (geocaches/importers/gpx_common.py)
        # rather than leaving iso_country_code to wait on a separate
        # Nominatim enrichment pass, which the country filter relies on
        # exclusively.
        fields["iso_country_code"] = name_to_iso(fields["country"])
