...
UI (read-only rendering + form choices) and the website automation in
``geocaches.sync.gc_web.notify``. No access methods here, so it stays in the public tree.
"""
...
# GC's account-wide instant-notification cap (informational only — GC's own
# NotificationFormUnavailable error is the actual enforcement point).
GC_NOTIFICATION_CAP = 40
