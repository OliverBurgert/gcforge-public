...
def fetch_recent_gc_logs(gc_client, code: str, count: int = 50) -> tuple[int, int]:
    ...
    normalized, api_count = gc_client.get_cache_logs_page(code, skip=0, take=count)
    if not api_count:
        return 0, 0
    ...
    logger.info("fetch_recent_gc_logs %s: API returned %d, saved %d new", code, api_count, saved)
    return saved, api_count
...
def fetch_more_gc_logs(gc_client, code: str, skip: int = 0, count: int = 50) -> tuple[int, int]:
    ...
    normalized, api_count = gc_client.get_cache_logs_page(code, skip=skip, take=count)
    if not api_count:
        return 0, 0
    ...
    if upgraded:
        logger.info("Upgraded %d GC log source_id(s) from numeric to API ref for %s", upgraded, code)
    saved = _save_logs(cache, new_logs)
    logger.info("fetch_more_gc_logs %s: skip=%d, API returned %d, saved %d new", code, skip, api_count, saved)
    return saved, api_count
...
def fetch_all_gc_logs(gc_client, code: str) -> int:
    ...
    while True:
        normalized, api_count = gc_client.get_cache_logs_page(code, skip=skip, take=_BATCH_SIZE)
        if not api_count:
            break
...
def fetch_oc_logs(oc_client, code: str, count: int = 50) -> int:
    ...
    for log in normalized:
        if log["source_id"] in existing_source_ids:
            _refresh_log_fields(cache, log["source_id"], log.get("text", ""), log["log_type"])
            continue
        ...
        if not log["source_id"].isdigit():
            ...
            if (log_date, log["user_name"]) in existing_numeric_date_user:
                ...
                _refresh_log_text_by_match(
                    cache, oc_client.platform, log_date, log["user_name"],
                    log["log_type"], log.get("text", ""),
                )
                upgraded += 1
                continue
        new_logs.append(log)
...
def ensure_my_gc_logs(gc_client, code: str) -> int:
    ...
    # Already have a found log from me? Use GC-only accounts throughout.
...
def _dedup_gc_logs(cache, normalized: list[dict], existing_ids: set) -> tuple[list[dict], int]:
    ...
    For logs already in the DB (matched by source_id), refresh the stored
    ``text`` and ``log_type`` if the API returned different values — covers
    both encoding fixes from earlier GPX imports and user-edited log types
    (e.g. DNF changed to Found it on geocaching.com).
    Locally-created logs (``is_local=True``) are never overwritten.

    Returns (new_logs, upgraded_count).
    """
    from datetime import date as _date

    # Build fallback lookups for logs with numeric source_ids:
    # - exact: (date, user_name, log_type) → always safe to match
    # - by_date_user: (date, user_name) → count, so we can match unambiguously
    #   when the user edited their log type (e.g. DNF → Found it) and there is
    #   only one log from that user on that date.
    existing_logs = list(
        cache.logs.filter(source="gc").values_list("source_id", "logged_date", "user_name", "log_type")
    )
    numeric_date_user_type = set()
    numeric_date_user_count: dict = {}
    for sid, dt, uname, lt in existing_logs:
        if sid.isdigit():
            numeric_date_user_type.add((dt, uname, lt))
            numeric_date_user_count[(dt, uname)] = numeric_date_user_count.get((dt, uname), 0) + 1
    ...
    for log in normalized:
        ...
        if not log["source_id"].isdigit():
            ...
            # Exact match (log_type unchanged)
            matched = (log_date, user_name, log["log_type"]) in numeric_date_user_type
            # Broader match: log_type changed but this user only has one log on this day
            if not matched and numeric_date_user_count.get((log_date, user_name), 0) == 1:
                matched = True
            if matched:
                _upgrade_gc_source_id(
                    cache, log_date, user_name, log["log_type"], log["source_id"],
                )
                _refresh_log_text_by_match(
                    cache, "gc", log_date, user_name,
                    log["log_type"], log.get("text", ""),
                )
                upgraded += 1
                continue
...
def _refresh_log_fields(cache, source_id: str, fresh_text: str, fresh_log_type: str) -> None:
    """Update Log.text and Log.log_type for a fetched log if the API value differs.

    Covers the case where the user edited their log type online (e.g. DNF → Found it).
    No-op if both values are already correct. Logs without a source_id are never touched.
    """
    if not source_id:
        return
    from geocaches.db_lock import db_write
    with db_write():
        Log.objects.filter(geocache=cache, source_id=source_id).update(
            text=fresh_text, log_type=fresh_log_type,
        )


def _refresh_log_text_by_match(cache, source, log_date, user_name, log_type, fresh_text):
    """Refresh text and log_type on the row whose source_id was just upgraded."""
    if log_date is None:
        return
    from geocaches.db_lock import db_write
    with db_write():
        Log.objects.filter(
            geocache=cache, source=source,
            logged_date=log_date, user_name=user_name,
        ).update(text=fresh_text, log_type=log_type)
...
def _upgrade_gc_source_id(cache, logged_date, user_name, log_type, new_source_id):
    ...
    log_type is intentionally not used as a filter — it may have changed since
    the GPX import (e.g. DNF → Found it), which is exactly the case we want to
    catch and upgrade.
    """
    from geocaches.db_lock import db_write
    with db_write():
