...
polite_delay(): jittered sleep for web-scraping call sites (geocaches/sync/gc_web/*).
"""
...
def polite_delay(lo: float = 2.0, hi: float = 5.0) -> None:
    """Sleep a random interval between web-scraping requests.

    Every new geocaching.com web/tRPC call site should call this between
    requests — this ships to each user's own installation rather than
    running centrally, so per-client politeness keeps aggregate load looking
    like normal browsing rather than a scraper farm. Same interval already
    used ad hoc in gcprivate/labs_client.py, factored out here for reuse.
    """
...
class QuotaTracker:
    ...
    # Known OC platforms — single source of truth: UserAccount.oc_platforms()
    # (every non-GC platform in PLATFORM_CHOICES; matches okapi_client._OC_NODES).
    _KNOWN_OC_PLATFORMS = UserAccount.oc_platforms()
    ...
    def _default_limit(platform: str, mode: str) -> int:
        ...
        if platform == "al":
            return 999_999  # AL API has no documented quota
