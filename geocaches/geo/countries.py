...
_ADMIN_AFFIXES: dict[tuple[str, str], list[str]] = {
    ...
    # Germany — strip only the "Stadt X" (kreisfreie Stadt) prefix so a city
    # stores its bare name ("München", "Karlsruhe").  "Landkreis "/"Kreis " are
    # deliberately KEPT: a kreisfreie Stadt and its surrounding Landkreis are
    # distinct legal entities ("München" vs "Landkreis München") that must not
    # collapse to the same value.
    ("DE", "county"): ["Stadt "],  # prefix
}
...
def all_countries() -> list[tuple[str, str]]:
    """Every ISO 3166-1 country as ``(alpha_2, English name)``, sorted by name."""
    countries = [
        (c.alpha_2, getattr(c, "common_name", None) or c.name)
