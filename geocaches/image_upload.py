...
def upload_images_to_gc_trackable_log(tb_log_ref: str, attachments: list[ImageAttachment]) -> list[ImageUploadResult]:
    """Upload images to a GC trackable log. Mirror of upload_images_to_gc.

    Endpoint: POST /v1/trackablelogs/{ref}/images, same body shape as the
    cache-log image endpoint.
    """
    from gcprivate.gc_client import GCClient
    import requests as _requests
    ...
    client = GCClient()
    access_token = client._api.access_token
    url = f"https://api.groundspeak.com/v1/trackablelogs/{tb_log_ref}/images"
    results = []

    for att in attachments:
        r = ImageUploadResult(filename=att.filename)
        try:
            processed_bytes, _mime = process_image(att)
            b64 = base64.b64encode(processed_bytes).decode("ascii")
            payload = {"base64ImageData": b64}
            if att.title:
                payload["name"] = att.title[:100]
            ...
            if resp.status_code == 201:
                ...
                r.error = f"TB image upload failed ({resp.status_code}): {resp.text[:200]}"
                logger.warning("TB image upload error for %s: %s", tb_log_ref, r.error)
