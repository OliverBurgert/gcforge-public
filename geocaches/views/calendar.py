...
def calendar_add_missing(request):
    ...
    # The Home tab's Type select has no way to pass "alc" explicitly, so also
    # infer it from the type itself — Adventure Lab always uses stage-find data.
    alc = request.POST.get("alc") == "1" or cache_type == CacheType.LAB
...
@require_POST
def calendar_remove_missing(request):
    cache_type = (request.POST.get("stat_type") or "").strip() or None
    res = calendar_service.remove_missing_days(cache_type)
    if res["removed"]:
        msg = ngettext(
            "Removed %(n)d %(type)s to-do.", "Removed %(n)d %(type)s to-dos.", res["removed"]
        ) % {"n": res["removed"], "type": res["type_label"]}
    else:
        msg = _("No matching to-dos to remove.")
    return _render_agenda(request, msg)
