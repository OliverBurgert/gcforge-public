...
from geocaches.importers.lookups import VALID_WP_PREFIXES
from ._common import _get_cache
...
def waypoint_save(request, gc_code, wp_id=None):
    ...
    prefix_raw = request.POST.get("prefix", "").strip().upper()
    prefix = prefix_raw if prefix_raw in VALID_WP_PREFIXES else ""
    ...
    if wp_id:
        ...
        wp.save(update_fields=["waypoint_type", "prefix", "name", "note", "latitude", "longitude", "is_user_modified"])
    else:
