...
from ..filtering.query import mine_q, mine_finder_q
...
def tools_misplaced_codes(request):
    ...
    if request.method == "POST":
        ...
        if action == "fix_all":
            ...
            for item in items:
                ...
                if item["correct_record"]:
                    # _merge_into drops the source row before touching the
                    # winner's codes, so the misplaced code needs no clearing.
        ...
        if action == "move":
            # The page may have been open while the correct record appeared.
            if live_code_conflict("oc_code", oc_code, exclude_pk=target.pk):
                messages.error(request, _("%(code)s already belongs to another cache — merge instead.") % {"code": oc_code})
            else:
