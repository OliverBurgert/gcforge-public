...
from ._common import _filtered_qs, _get_cache
...
def cache_set_found_accuracy(request, gc_code):
    """Set found_accuracy on a single Adventure Lab stage. POST only."""
    from geocaches.models import FoundAccuracy

    if request.method != "POST":
        return redirect("geocaches:detail", gc_code=gc_code)

    cache = _get_cache(gc_code)
    value = request.POST.get("found_accuracy", "")
    if value and value not in {c.value for c in FoundAccuracy}:
        return HttpResponseBadRequest(f"Invalid found_accuracy: {value!r}")
    cache.found_accuracy = value
    cache.save(update_fields=["found_accuracy"])
    return redirect("geocaches:detail", gc_code=cache.display_code)
    ...
    try:
        if source == "gc" and cache.gc_code:
            ...
            if action == "fetch_recent":
                saved, api_count = gc_access.fetch_recent_gc_logs(cache.gc_code, count=batch_size)
                skip_used = batch_size
            elif action == "fetch_more":
                skip_used = int(request.POST.get("skip", 0))
                saved, api_count = gc_access.fetch_more_gc_logs(cache.gc_code, skip=skip_used, count=batch_size)
                skip_used += batch_size
            elif action == "fetch_all":
                saved = gc_access.fetch_all_gc_logs(cache.gc_code)
                api_count = 0  # exhausted
            elif action == "fetch_mine":
                # Always via the website's showOwnerOnly=true path, regardless of
                # gc_access_mode — choosing this button already is the backend
                # choice (same precedent as "Fetch by List Generation").
                from geocaches.sync.gc_web.log_fetch import ensure_my_gc_logs
                saved = ensure_my_gc_logs(cache.gc_code)
                api_count = 0  # exhausted
...
def cache_refresh(request, gc_code):
    ...
    if source == "al" and cache.al_code and cache.adventure_id:
        ...
        try:
            ...
            from geocaches.sync import gc_access
            gc_access.ensure_my_gc_logs(cache.gc_code)
        except Exception as exc:
...
def cache_enrich(request):
    ...
    if fields_param in ("location_offline", "location_offline_update"):
        ...
        started = True
    elif fields_param.startswith("found_accuracy_"):
        # Instant bulk field-set, not a background enrichment fetch.
        # NB: `qs, _ = _filtered_qs(request)` above shadows the module-level
        # gettext `_` — use the unaliased import for any singular message here.
        from django.contrib import messages
        from django.utils.translation import gettext
        from geocaches.models import FoundAccuracy
        from geocaches.services.found_accuracy import bulk_set_found_accuracy

        suffix = fields_param[len("found_accuracy_"):]
        value = "" if suffix == "clear" else suffix
        if value and value not in {c.value for c in FoundAccuracy}:
            return HttpResponseBadRequest(f"Invalid found_accuracy: {value!r}")

        changed = bulk_set_found_accuracy(qs, value)
        if changed:
            messages.success(request, ngettext(
                "Set found accuracy on %(n)d Adventure Lab find.",
                "Set found accuracy on %(n)d Adventure Lab finds.",
                changed,
            ) % {"n": changed})
        else:
            messages.info(request, gettext("No Adventure Lab finds to update."))
        started = True
    else:
...
def cache_update_location_from_point(request, gc_code):
    """POST lat/lon — locate country/state/county for that point and save it
    as a manual location override (same semantics as ``cache_save_location``).

    Local-first: point-in-polygon against already-downloaded boundaries
    (``offline_enrich.locate_point`` — no network). Any field it can't
    resolve (deeper boundary tier not downloaded for that country) is filled
    from Nominatim. JSON response for the map's left-click menu.
    """
    if request.method != "POST":
        return HttpResponseNotAllowed(["POST"])
    from django.contrib import messages
    from geocaches.enrichment import fetch_location
    from geocaches.services import offline_enrich
    ...
    loc = offline_enrich.locate_point(lat, lon)
    source = "local"
    if not (loc.get("iso_country_code") and loc.get("state") and loc.get("county")):
        online = fetch_location(lat, lon)
        if online:
            source = "local+online" if loc else "online"
            for key, value in online.items():
                if value and not loc.get(key):
                    loc[key] = value

    if not loc.get("iso_country_code"):
        return JsonResponse({"ok": False, "error": _("No location data found for this point.")})

    cache.iso_country_code = loc.get("iso_country_code", "")
    cache.country = loc.get("country", "")
    cache.state = loc.get("state", "")
    cache.county = loc.get("county", "")
    cache.manual_location = True
    cache.save(update_fields=[
        "iso_country_code", "country", "state", "county", "manual_location",
    ])
    messages.success(request, _("Updated location for %(code)s from the map (%(source)s).") % {
        "code": cache.display_code, "source": source,
    })
    return JsonResponse({
        "ok": True,
        "country": cache.country, "state": cache.state, "county": cache.county,
        "source": source,
    })


def cache_update_elevation_from_point(request, gc_code):
    """POST lat/lon — fetch elevation for that point and save it as
    ``elevation_user`` (protected override, same field the manual edit dialog
    uses). Local-first: SRTM tiles (no network); OpenTopoData online only if
    the local tile is void/missing. JSON response for the map's left-click menu.
    """
    if request.method != "POST":
        return HttpResponseNotAllowed(["POST"])
    from django.contrib import messages
    from geocaches.enrichment import fetch_elevation, fetch_elevation_online
    ...
    source = "local"
    if ele is None:
        ele = fetch_elevation_online(lat, lon)
        source = "online"
    if ele is None:
        return JsonResponse({"ok": False, "error": _("No elevation data found for this point.")})

    cache.elevation_user = ele
    cache.save(update_fields=["elevation_user"])
    messages.success(request, _("Updated elevation for %(code)s from the map (%(source)s).") % {
        "code": cache.display_code, "source": source,
    })
    return JsonResponse({"ok": True, "elevation": ele, "source": source})
