...
def tools_remove_unattended_past_events(request):
    ...
    if request.method == 'POST':
        add_to_ignore_list = request.POST.get('add_to_ignore_list') == 'on'
        events = list(past_events)
        count = len(events)

        if add_to_ignore_list:
            from ..services.ignore_list import add_internal
            for ev in events:
                code = ev.display_code
                if code:
                    add_internal(code, notes="Removed as unattended past event")

        Geocache.objects.filter(pk__in=[ev.pk for ev in events]).delete()

        if add_to_ignore_list:
            messages.success(request, ngettext(
                "Deleted %(n)d past event cache and added it to the local ignore list.",
                "Deleted %(n)d past event caches and added them to the local ignore list.",
                count,
            ) % {"n": count})
        else:
            messages.success(request, ngettext(
