...
# Sentinel for the tag filter's "no tag at all" option — distinct from ""
# (which means "no filter applied", i.e. every trackable regardless of tags).
NO_TAG_VALUE = "__none__"
...
def _read_filters(params) -> dict:
    """Read the filter fields out of a GET or POST QueryDict.

    ``params`` is ``request.GET`` for the page views (filters live in the
    URL) and ``request.POST`` for ``trackable_refresh_filtered_plan`` (the
    JS posts ``window.location.search`` verbatim as the body — same
    convention as ``trackable_resolve_locations``).
    """
    return {
...
def _apply_filters(qs, fv: dict, gc_username: str):
    if fv["q"]:
        qs = qs.filter(
            ...
            | Q(tracking_code__icontains=fv["q"])
        )
    if fv["state"]:
...
def trackable_sync_plan(request):
    ...
    try:
        client = get_trackable_api_client()
        if scope == "mine":
            items = (
...
@require_POST
def trackable_refresh_filtered_plan(request):
    """Return the refs matching the *currently applied* list filters, plus
    which of them should get the discovered/moved auto-tag on refresh.

    POST body carries the same filter fields as the list view's query string
    (q, state, kind, series, tag, mine, held) — the JS posts
    ``window.location.search`` verbatim, same convention as
    ``trackable_resolve_locations``. No pagination cap: "Refresh filtered"
    means the whole filtered set, however large.

    Tag membership is resolved once here (not per-TB) against the same
    discovered/moved ref lists ``trackable_sync_plan`` uses, then intersected
    with the filtered refs — ``trackable_sync_one`` applies whichever of
    "discovered"/"moved" apply (comma-joined) as it refreshes each one. A TB
    matching neither (never discovered or moved by this account) just gets
    refreshed with no tag — that's expected, not an error.
    """
    fv = _read_filters(request.POST)
    gc_username = _resolve_gc_username()
    qs = _apply_filters(Trackable.objects.all(), fv, gc_username)
    refs = list(qs.values_list("reference_code", flat=True))
    ...
    if refs:
        try:
            ...
            client = get_trackable_api_client()
            ref_set = set(refs)
            discovered = sorted(ref_set & set(client.get_my_discovered_refs()))
            moved = sorted(ref_set & set(client.get_my_moved_refs()))
        except Exception as exc:  # noqa: BLE001
            # Non-fatal — the refresh itself (sync_trackable, gc_access-aware
            # with a web fallback) doesn't need this; only the auto-tagging
            # does, and it's fine for a TB to end up untagged.
            logger.warning("trackable_refresh_filtered_plan: discovered/moved lookup failed: %s", exc)
...
def trackable_tracking_code_fetch(request):
    ...
    try:
        from geocaches.sync.gc_web.tb_tracking import fetch_tracking_code
...
def trackable_sync_one(request):
    ...
    Optional ``tag`` POST field attaches a Tag (auto-created) to the newly
    synced trackable — a comma-separated list applies more than one (used by
    "Refresh filtered", where a TB can be both discovered and moved; every
    other caller just sends a single name). Used by the discovered/moved
    sync flows and by refresh-filtered.
    """
    ...
    tag_field = (request.POST.get("tag") or "").strip()
    if tag_field:
        from geocaches.models import Tag
        for name in [t.strip() for t in tag_field.split(",") if t.strip()]:
