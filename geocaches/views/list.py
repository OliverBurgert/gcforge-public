...
from ..filtering.filter_expr import Condition, Group, OP_AND, OP_OR, to_url_param
from ..models import CacheSize, CacheStatus, CacheType, FoundAccuracy, Geocache, LogType, SavedFilter, SavedWhereClause, Tag
from ..filtering.query import apply_all, build_filter_chips, mine_q
from ..runtime_cache import get_attribute_groups, get_country_list
...
def cache_list(request):
    from preferences.views.settings import get_active_columns, get_active_preset_name
    ...
    from ..filtering.query import tree_to_toolbar_state
    ...
    # --- filter option lists (toolbar dropdowns need these at initial render;
    # the dialog-only lists — attributes, saved-filter trees, where-clause
    # history — are fetched lazily, see filter_dialog_options() below) ---
    tags = Tag.objects.select_related("default_ref_point").order_by("name")
    _country_data = get_country_list()
    countries = _country_data["countries"]
    has_no_country = _country_data["has_no_country"]
    has_untagged = Geocache.objects.filter(tags__isnull=True).exists()
    has_accounts = bool(_accounts)

    # --- saved filters (toolbar "Saved filter" dropdown) ---
...
def filter_dialog_options(request):
    """Body content for the three "Custom filter" dialogs (WP-04).

    Fetched once per page load, the first time any of the dialogs is opened
    (see ``_filter_dialogs_shell.html`` / ``static/js/cache-list.js``). Option
    lists that don't depend on the current filter (cache types/sizes/statuses,
    tags, countries, attributes, saved-filter trees, where-clause history) are
    the same regardless of when this is called; the current-filter-dependent
    bits (current tree, active where-clause) are derived from ``qs`` — the
    query string active on the page at the moment the dialog was opened,
    forwarded by the client since a lazy fetch can happen well after this
    request's own querystring was current.
    """
    params = QueryDict(request.GET.get("qs", "").lstrip("?"))
    # apply_all() only builds a lazy queryset (never evaluated below) plus the
    # fv dict; Geocache.objects.all() (rather than .none()) keeps
    # apply_where_clause()'s qs.explain() validation working exactly as it
    # does for the main list view.
    _, fv = apply_all(Geocache.objects.all(), params)

    tags = Tag.objects.select_related("default_ref_point").order_by("name")
    countries = get_country_list()["countries"]
    attrs_by_source = get_attribute_groups()
    ...
    if fv.get("fx"):
        try:
            from ..filtering.filter_expr import from_url_param
    ...
    return render(request, "geocaches/partials/_filter_dialog_options.html", context)
