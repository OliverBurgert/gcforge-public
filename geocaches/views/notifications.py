...
def _annotate(n) -> None:
    ...
    n.out_of_sync = svc.is_out_of_sync(n)
...
def _group_by_location(qs):
    ...
    for key, rows in sorted(grouped.items(), key=sort_key):
        ...
        out.append({
            ...
            "server_linked_count": sum(1 for n in rows if n.server_id),
        })
    return out
...
def notifications_page(request):
    ...
    return render(request, "geocaches/notifications.html", {
        "groups": groups,
        "total": qs.count(),
        "active_on_server": qs.exclude(server_id="").count(),
        "gc_notification_cap": notify_constants.GC_NOTIFICATION_CAP,
...
@require_POST
def notification_unlink(request, pk):
    """Delete the server-side copy only, keeping the local row as a template."""
    n = get_object_or_404(GCNotification, pk=pk)
    name = n.name
    try:
        svc.unlink_from_server(n.id)
    except Exception as exc:
        logger.exception("Unlink failed")
        messages.error(request, _("Removing from geocaching.com failed: %s") % exc)
        return redirect(reverse("geocaches:notifications"))
    messages.success(request, _("Removed '%s' from geocaching.com — kept locally as a template.") % name)
    return redirect(reverse("geocaches:notifications"))
...
@require_POST
def notifications_region_unlink(request):
    location_id = _parse_location_id(request.POST.get("location_id", ""))
    try:
        result = svc.unlink_by_location(location_id)
    except Exception as exc:
        logger.exception("Region unlink failed")
        messages.error(request, _("Bulk removal from geocaching.com failed: %s") % exc)
        return redirect(reverse("geocaches:notifications"))
    if result["errors"]:
        messages.warning(request, _("Removed %(n)d from geocaching.com; %(errs)d error(s).") % {
            "n": result["unlinked"], "errs": len(result["errors"]),
        })
    else:
        messages.success(request, _("Removed %d notification(s) from geocaching.com — kept locally as templates.") % result["unlinked"])
    return redirect(reverse("geocaches:notifications"))
...
def notifications_alt_emails(request):
    from geocaches.sync.gc_web import notify as notify_web
    try:
