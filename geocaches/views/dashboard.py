...
def dashboard(request):
    ...
    context = {
        ...
        "type_options": stats_service.type_options(),
        # Calendar's fill/remove-missing-days controls always offer Adventure
        # Lab, regardless of the stats "Include Adventure Lab caches" toggle.
        "calendar_type_options": stats_service.type_options(always_include_al=True),
        "minimum_choices": range(1, 11),
...
def dashboard_statistics(request):
    ...
    return render(request, "geocaches/partials/_dashboard_statistics.html", {
        ...
        "ref_points": list(ReferencePoint.objects.values("id", "name")),
        "by_attribute": stats_service.finds_by_attribute(),
    })
...
def dashboard_souvenirs_list(request):
    ...
    params (repeatable ids) + ``untagged`` checkbox filter which show. The
    ``known_tag`` hidden inputs (one per tag rendered last time) tell tags
    created since then apart from tags the user actually unchecked, so a
    brand-new tag defaults to selected instead of vanishing its souvenirs.
    """
    ...
    if request.GET.get("tagfilter"):
        ...
        known = {int(t) for t in request.GET.getlist("known_tag") if t.isdigit()}
        selected |= {tid for tid in all_ids if tid not in known}
        untagged = request.GET.get("untagged") == "1"
        tag_ids = list(selected)
    else:
