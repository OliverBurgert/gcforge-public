...
stores; the filter chain in ``geocaches/filtering/filters.py`` MUST NOT touch either.
...
def bulk_set(qs, state: str, session) -> dict:
    ...
    # Parents carry the is_al_parent flag; stages have al_detail.
    parent_adv_ids = list(
        qs.filter(is_al_parent=True)
