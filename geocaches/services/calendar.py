...
def remove_missing_days(cache_type: str | None) -> dict:
    """Delete the missing-day to-dos for one cache type (reverts
    :func:`add_missing_days`).

    Mirrors the fill action's Type select: an empty/``None`` ``cache_type``
    targets the "any cache" bucket.  Returns ``{"removed", "type_label",
    "total"}``.
    """
    from django.utils.translation import gettext as _
    ...
    any_label = _("any cache")
    type_label = dict(CacheType.choices).get(cache_type, any_label) if cache_type else any_label
    removed = CalendarEntry.objects.filter(
        kind=CalendarEntry.KIND_MISSING_DAY, cache_type=cache_type or "",
    ).delete()[0]
    return {
        "removed": removed,
