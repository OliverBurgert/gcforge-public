...
def merge_duplicate_adventure(old_adv, canonical_adv) -> None:
    ...
    old_parent = old_adv.stages.filter(is_al_parent=True).prefetch_related("tags").first()
    canonical_parent = canonical_adv.stages.filter(is_al_parent=True).first()
...
def _upsert_parent_geocache(
    ...
    with db_write_atomic():
        ...
        # If *every* geocache belonging to `adv` was soft-deleted together
        # (the whole adventure was trashed, see services.trash) and the purge
        # above just removed the last of them, signals.cleanup_orphan_adventure
        # deletes `adv` itself as a side effect — leaving the in-memory `adv`
        # object pointing at a row that's no longer there. Resurrect it with
        # the same pk before attaching the new parent geocache to it, or the
        # get_or_create() below fails with a FOREIGN KEY error every time
        # this adventure is re-imported (2026-09-07, confirmed live).
        if adv.pk and not Adventure.objects.filter(pk=adv.pk).exists():
            adv.save(force_insert=True)
...
_AL_DETAIL_FIELD_MAP = {
    ...
    "al_youtube_video_id":    "youtube_video_id",
...
def save_alc_stage(al_code: str, model_fields: dict, tags: list, stats):
    ...
    from geocaches.models import ALStageDetail
    from geocaches.services.save import save_geocache as _save
...
def save_adventure_from_api(data: dict, tags=None, found_accuracy: str = ""):
    """
    Upsert an adventure and all its stages from AL API normalized data.

    ``found_accuracy`` (see ``geocaches.models.FoundAccuracy``), when given, is
    stamped only on stages this call newly marks found — it never overwrites
    an accuracy already recorded on a stage found by an earlier import.
    ...
    for stage in data.get("stages", []):
        ...
        model_fields = {
            ...
            "al_youtube_video_id":  stage.get("youtube_video_id", ""),
        }
        ...
        try:
            ...
            if gc and stage.get("is_complete") and not gc.found:
                ...
                update_fields = ["found"]
                if found_accuracy:
                    gc.found_accuracy = found_accuracy
                    update_fields.append("found_accuracy")
...
def parent_geocache_for_adventure(adv):
    ...
    if adv is None:
        return None
    return adv.stages.filter(is_al_parent=True).first()
