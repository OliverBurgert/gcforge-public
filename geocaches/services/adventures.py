...
def is_al_parent(geocache) -> bool:
    """True if this cache is an Adventure Lab parent (the synthetic adventure row).

    Reads the denormalised ``Geocache.is_al_parent`` flag, which every AL writer
    maintains — previously this was derived from the absence of an ALStageDetail
    row, costing a query per call whenever the relation wasn't selected.
    """
    return bool(geocache.is_al_parent)
...
def recompute_adventure_completed(adventure) -> bool:
    ...
    parent = adventure.stages.filter(is_al_parent=True).first()
    if parent is None:
        return all_found
    ...
    if all_found:
        ...
        if completion_date:
            target_date = timezone.localtime(completion_date).date()
