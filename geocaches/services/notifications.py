...
``geocaches.sync.gc_web.notify``.  Higher-level operations: full sync, bulk
...
def _notify_web():
    """Lazy handle to the notify.py web-scraping automation."""
    from geocaches.sync.gc_web import notify
    return notify
...
def sync_with_server(task_info=None) -> dict:
    ...
    * Local-only and server-deleted entries are returned for user decision —
    ...
    for nid, n in list(local_by_nid.items()):
        ...
        if all(light.values()):
            result.matched += 1
            # Heal a stale hash left behind by an update that didn't touch it
            # (e.g. a toggle from before that bookkeeping existed) — cheap,
            # no network call, since we already have both sides in hand.
            current_hash = _compute_server_hash(_to_server_payload(n))
            if n.server_hash != current_hash:
                n.server_hash = current_hash
                from geocaches.db_lock import db_write
                with db_write():
                    n.save(update_fields=["server_hash"])
            continue
...
def unlink_from_server(notification_id: int) -> GCNotification:
    """Delete the server-side notification only, keeping the local row as a
    reusable template (frees a slot against GC's account-wide cap)."""
    n = GCNotification.objects.get(id=notification_id)
    if n.server_id:
        _notify_web().delete_notification(n.server_id)
        logger.info("GC notification deleted from server (kept local): NID=%s %r (%s)",
                    n.server_id, n.name, _type_label(n.cache_type_id))
        n.server_id = ""
        n.last_synced_at = None
        n.server_hash = ""
        from geocaches.db_lock import db_write
        with db_write():
            n.save(update_fields=["server_id", "last_synced_at", "server_hash"])
    return n


def is_out_of_sync(n: GCNotification) -> bool:
    """Whether the local row has drifted from the state we last synced with."""
    return bool(n.server_id) and bool(n.server_hash) and (
        n.server_hash != _compute_server_hash(_to_server_payload(n))
    )
...
def toggle_enabled(notification_id: int) -> bool:
    ...
    if n.server_id:
        ...
        n.last_synced_at = datetime.now(timezone.utc)
        n.server_hash = _compute_server_hash(_to_server_payload(n))
        update_fields += ["last_synced_at", "server_hash"]
    ...
    for i, n in enumerate(targets):
        try:
            if n.server_id:
                ...
                _push_create(n)
            pushed += 1
        except _notify_web().NotificationFormUnavailable as exc:
            # GC's create form is gone (almost always the account-wide cap).
            # Retrying the remaining rows is futile — record them and stop.
            logger.warning("Push-all stopped at %r: %s", n.name, exc)
            for rest in targets[i:]:
                errors.append({"id": rest.id, "name": rest.name, "error": str(exc)})
            if task_info:
                task_info.completed = len(targets)
            break
        except Exception as exc:
    ...
    *,
    ...
    for i, type_id in enumerate(type_ids):
        ...
        try:
            ...
            # GC's create form is gone (almost always the 40-notification cap).
            # Retrying the remaining types is futile — record this one and stop.
            logger.warning("Bulk-create aborted at type %s: %s", type_id, exc)
            errors.append({"type_id": type_id, "type_name": type_name, "error": str(exc)})
            for rest in type_ids[i:]:
                if rest == type_id:
                    continue
                errors.append({
                    "type_id": rest,
                    "type_name": notify_constants.CACHE_TYPES.get(rest, str(rest)),
                    "error": str(exc),
                })
            if task_info:
                task_info.completed = len(type_ids)
            break
        except Exception as exc:
...
def unlink_by_location(location_id: int | None, task_info=None) -> dict:
    """Delete every server-linked notification in a group from GC.com only,
    keeping each local row as a template."""
    qs = _location_filter(location_id).exclude(server_id="")
    targets = list(qs)
    if task_info:
        task_info.phase = f"Unlinking {len(targets)} notification(s) from server"
        task_info.total = len(targets)
        task_info.completed = 0
    ...
    for i, n in enumerate(targets):
        try:
            ...
            logger.warning("Unlink failed for NID=%s: %s", n.server_id, exc)
            errors.append({"nid": n.server_id, "name": n.name, "error": str(exc)})
        if task_info:
            task_info.completed = i + 1
