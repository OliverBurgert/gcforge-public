...
def _gc_action(web_fn, api_fn):
    """Run ``api_fn`` (official-API mutation/fetch) unless ``web_preferred``,
    the API build is unavailable, or it raises — in which case run
    ``web_fn`` instead. Same ``api_preferred``/``web_preferred`` convention
    as ``geocaches.sync.gc_access``. Returns whichever function ran returned.
    """
    ...
    if gc_api_available():
        try:
            ...
            logger.warning("GC ignore-list action via official API failed, falling back to web: %s", exc)

    return web_fn()


def _replace_gc_entries(remote: list[dict]) -> None:
    from geocaches.db_lock import db_write_atomic
    with db_write_atomic():
...
def sync_gc_ignore_list() -> int:
    """Full resync of the GC ignore list via the active backend.

    See ``geocaches.sync.gc_access``'s ``api_preferred``/``web_preferred``
    convention: tries the official API first (falling back to the website on
    any failure), or always uses the website when preferred.
    """
    def _api() -> int:
        from geocaches.sync.gc_access import get_gc_client
        remote = get_gc_client().get_ignore_list()
        _replace_gc_entries(remote)
        return len(remote)

    def _web() -> int:
        from geocaches.sync.gc_web.ignore_list import list_ignored_gc_web
        remote = list_ignored_gc_web()
        _replace_gc_entries(remote)
        return len(remote)

    return _gc_action(_web, _api)


def add_gc(gc_code: str) -> IgnoreListEntry:
    """Add a geocache to the account's GC ignore list via the active backend."""
    def _api():
        from geocaches.sync.gc_access import get_gc_client
        get_gc_client().add_to_ignore_list(gc_code)

    def _web():
        from geocaches.sync.gc_web.ignore_list import add_gc_web
        add_gc_web(gc_code)

    _gc_action(_web, _api)
...
def remove_gc(gc_code: str) -> bool:
    """Remove a geocache from the account's GC ignore list via the active backend."""
    def _api():
        from geocaches.sync.gc_access import get_gc_client
        get_gc_client().remove_from_ignore_list(gc_code)

    def _web():
        from geocaches.sync.gc_web.ignore_list import remove_gc_web
        remove_gc_web(gc_code)
