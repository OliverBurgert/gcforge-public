"""
Tests for geocaches/filtering/filter_expr.py — phase 1 of the v2 filter system.
...
from geocaches.filtering.filter_expr import (
    MAX_DEPTH,
    OP_AND,
    OP_NOT,
    OP_OR,
...
from geocaches.filtering.query import apply_all, apply_filter_expr
...
class ConditionCompilerTests(TestCase):
    ...
    def test_found_accuracy_in_real_value(self):
        self.c2.found_accuracy = "app_verified"
        self.c2.save(update_fields=["found_accuracy"])
        self.assertEqual(
            self._ids(Condition("found_accuracy", "in", ["app_verified"])),
            {"GC0002"},
        )

    def test_found_accuracy_is_none_matches_blank(self):
        self.c2.found_accuracy = "app_verified"
        self.c2.save(update_fields=["found_accuracy"])
        # c1 and c3 are left blank (the field's default) by _make_cache.
        self.assertEqual(
            self._ids(Condition("found_accuracy", "is_none", True)),
            {"GC0001", "GC0003"},
        )

    def test_found_accuracy_in_combines_blank_with_real_value(self):
        # "" (Unknown) must survive alongside a real value in the same "in"
        # list — this is exactly what _register_enum's _normalise_list()
        # would silently drop, which is why found_accuracy is bespoke.
        self.c1.found_accuracy = "armchair"
        self.c1.save(update_fields=["found_accuracy"])
        self.c2.found_accuracy = "app_verified"
        self.c2.save(update_fields=["found_accuracy"])
        self.assertEqual(
            self._ids(Condition("found_accuracy", "in", ["", "armchair"])),
            {"GC0001", "GC0003"},
        )

    def test_found_accuracy_not_in_excludes_blank_when_selected(self):
        self.c1.found_accuracy = "armchair"
        self.c1.save(update_fields=["found_accuracy"])
        self.c2.found_accuracy = "app_verified"
        self.c2.save(update_fields=["found_accuracy"])
        self.assertEqual(
            self._ids(Condition("found_accuracy", "not_in", ["", "app_verified"])),
            {"GC0001"},
        )
    ...
    def test_date_day_month_day_only_matches_across_years_and_months(self):
        c4 = _make_cache("GC0004", found_date=date(2021, 2, 21))
        c5 = _make_cache("GC0005", found_date=date(2023, 3, 21))
        _make_cache("GC0006", found_date=date(2022, 12, 5))
        ids = self._ids(Condition("found_date", "day_month", {"day": 21}))
        self.assertEqual(ids, {c4.gc_code, c5.gc_code})

    def test_date_day_month_month_only_matches_any_year(self):
        c4 = _make_cache("GC0004", found_date=date(2021, 12, 1))
        c5 = _make_cache("GC0005", found_date=date(2023, 12, 25))
        _make_cache("GC0006", found_date=date(2022, 11, 5))
        ids = self._ids(Condition("found_date", "day_month", {"month": 12}))
        self.assertEqual(ids, {c4.gc_code, c5.gc_code})

    def test_date_day_month_both_matches_leap_day_across_years(self):
        c4 = _make_cache("GC0004", found_date=date(2020, 2, 29))
        c5 = _make_cache("GC0005", found_date=date(2024, 2, 29))
        _make_cache("GC0006", found_date=date(2021, 2, 28))
        _make_cache("GC0007", found_date=date(2020, 3, 29))
        ids = self._ids(Condition("found_date", "day_month", {"day": 29, "month": 2}))
        self.assertEqual(ids, {c4.gc_code, c5.gc_code})

    def test_date_day_month_empty_value_is_noop(self):
        _make_cache("GC0004", found_date=date(2020, 2, 29))
        ids = self._ids(Condition("found_date", "day_month", {}))
        self.assertEqual(ids, {"GC0001", "GC0002", "GC0003", "GC0004"})
...
class AlcConditionsTests(TestCase):
    ...
    Fixture layout:
      ...
      cache_adv_parent — the synthetic parent row for adv_alice: is_al_parent=True,
        cache_type="Adventure Lab", no al_detail (parents never get one)
    """
    ...
    def setUp(self):
        ...
        # Adventure Lab parent row for adv_alice: is_al_parent=True, no al_detail.
    ...
    # 1. is_adventure — is_al_parent flag (NOT cache_type; parents and stages
    # share cache_type="Adventure Lab", so only the flag distinguishes them)
    def test_is_adventure(self):
        result = self._ids(Condition("alc", "is_adventure", True))
        self.assertEqual(result, {"GCALP1"})

    def test_is_adventure_inside_not_group_excludes_only_the_parent(self):
        # NOT (is_adventure) must exclude the parent but keep every stage and
        # the ordinary cache — a group-level negation, not a per-field one.
        tree = Group(OP_AND, [
            Group(OP_NOT, [Condition("alc", "is_adventure", True)]),
        ])
        result = set(Geocache.objects.filter(compile_tree(tree)).values_list("gc_code", flat=True))
        self.assertNotIn("GCALP1", result)
    ...
    def test_is_stage(self):
        ...
        self.assertEqual(result, {"ALA001", "ALA002", "ALA003", "ALB001", "ALB002"})

    def test_is_stage_inside_not_group_excludes_only_stages(self):
        tree = Group(OP_AND, [
            Group(OP_NOT, [Condition("alc", "is_stage", True)]),
        ])
        result = set(Geocache.objects.filter(compile_tree(tree)).values_list("gc_code", flat=True))
    ...
    def test_in_progress_matches_legacy_filter(self):
        from geocaches.filtering.filters import apply_alc_in_progress_filter
        legacy_ids = set(
...
class ConditionToLabelTests(TestCase):
    ...
    def _check(self, field, op, value, expected_substr):
        from geocaches.filtering.filter_expr import condition_to_label
    ...
    def test_date_day_month(self):
        self._check("found_date", "day_month", {"day": 29, "month": 2}, "29. February")
        self._check("found_date", "day_month", {"day": 21}, "day 21")
        self._check("found_date", "day_month", {"month": 12}, "December")

    def test_found_accuracy(self):
        self._check("found_accuracy", "in", ["app_verified"], "App-verified")
        self._check("found_accuracy", "not_in", ["armchair"], "∉")
        self._check("found_accuracy", "in", ["", "armchair"], "Unknown")
        self._check("found_accuracy", "is_none", True, "Unknown")
...
class TreeChipRenderingTests(TestCase):
    ...
    def _fv_with_tree(self, tree):
        from geocaches.filtering.filter_expr import to_url_param
        return {"fx": to_url_param(tree), "fx_count": 0, "fx_error": "", "f_name": ""}

    def test_two_leaves_render_as_two_chips(self):
        from geocaches.filtering.filter_expr import Condition, Group, OP_AND
        from geocaches.filtering.query import build_filter_chips
        tree = Group(OP_AND, [
    ...
    def test_nested_group_renders_as_summary_chip(self):
        from geocaches.filtering.filter_expr import Condition, Group, OP_AND, OP_OR
        from geocaches.filtering.query import build_filter_chips
        tree = Group(OP_AND, [
            Condition("name", "contains", "alpha"),
            Group(OP_OR, [
...
class TreeToToolbarStateTests(TestCase):
    def test_extracts_single_value_dropdowns(self):
        ...
        from geocaches.filtering.query import tree_to_toolbar_state
        tree = Group(OP_AND, [
