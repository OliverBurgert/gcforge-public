...
class SaveAdventureFromApiTest(TestCase):
    ...
    def _complete_stage_data(self, **overrides):
        stage = {
            "al_stage_uuid": "stage-uuid-1",
            "stage_number": 1,
            "lat": 52.51,
        ...
        return _minimal_api_data(stages=[stage])

    def test_found_accuracy_stamped_on_newly_found_stage(self):
        adv, _ = save_adventure_from_api(self._complete_stage_data(), found_accuracy="app_verified")
        stage = Geocache.objects.get(adventure=adv, al_detail__isnull=False)
        self.assertTrue(stage.found)
        self.assertEqual(stage.found_accuracy, "app_verified")

    def test_found_accuracy_left_blank_when_not_given(self):
        adv, _ = save_adventure_from_api(self._complete_stage_data())
        stage = Geocache.objects.get(adventure=adv, al_detail__isnull=False)
        self.assertTrue(stage.found)
        self.assertEqual(stage.found_accuracy, "")

    def test_found_accuracy_not_overwritten_on_already_found_stage(self):
        adv, _ = save_adventure_from_api(self._complete_stage_data(), found_accuracy="app_verified")
        stage = Geocache.objects.get(adventure=adv, al_detail__isnull=False)
        stage.found_accuracy = "armchair"
        stage.save(update_fields=["found_accuracy"])

        save_adventure_from_api(self._complete_stage_data(), found_accuracy="visited_same_day")
        stage.refresh_from_db()
        self.assertEqual(stage.found_accuracy, "armchair")
...
class MergeDuplicateAdventureTest(TestCase):
    def _make_adventure_tree(self, code, stage_count=2, found_stages=None):
        ...
        parent = Geocache.objects.create(
            ...
            latitude=52.0, longitude=13.0, adventure=adv, is_al_parent=True,
        )
        stages = []
