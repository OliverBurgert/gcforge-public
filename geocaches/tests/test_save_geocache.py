...
class TestAlParentFoundGuard(TestCase):
    ...
    def test_rejects_found_on_al_parent_update(self):
        save_geocache(
            al_code="LCGUARD",
            fields=self._al_fields(adventure=self.adv, is_al_parent=True),
        )
        with self.assertRaises(ValueError):
            save_geocache(
                al_code="LCGUARD",
    ...
    def test_accepts_found_on_al_stage(self):
        # A stage is simply a row the parent flag was never set on.
        save_geocache(
            al_code="LCGUARD-1",
    ...
    def test_direct_save_no_longer_raises(self):
        ...
        cache = Geocache.objects.create(
            ...
            cache_type=CacheType.LAB,
            latitude=52.52, longitude=13.405,
            adventure=self.adv, is_al_parent=True,
        )
        cache.found = True
