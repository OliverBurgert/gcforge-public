...
from geocaches.models import CacheSize, CacheStatus, CacheType, DistanceCache, Geocache
...
class FindsByBearingTests(TestCase):
    """finds_by_bearing() must read distances through annotate_distance()'s
    coalescing annotation, not DistanceCache directly, so a partially (or
    entirely un-) filled cache can't under-count.

    See docs/architecture-review-2026-09.md §8.
    """
    ...
    def setUp(self):
        ...
        # Two finds in clearly distinct compass sectors (12 sectors of 30°
        # each below) so float-summation order can never affect a rounded
        # average: due north and due west of the reference point.
        _cache("GC1", found=True, latitude=48.5, longitude=9.0)
        _cache("GC2", found=True, latitude=48.0, longitude=8.0)
        # Unfound: must never be counted.
        _cache("GC3", found=False, latitude=48.2, longitude=9.2)

    def test_counts_every_find_with_empty_distance_cache(self):
        with patch("geocaches.geo.distance_cache.ensure_cached", return_value=False):
            data = stats.finds_by_bearing(self.ref.pk)
        self.assertEqual(DistanceCache.objects.count(), 0)  # fallback only, never filled
        self.assertEqual(data["total"], 2)
        self.assertEqual(sum(data["counts"]), 2)

    def test_agrees_with_cached_path_once_populated(self):
        with patch("geocaches.geo.distance_cache.ensure_cached", return_value=False):
            fallback = stats.finds_by_bearing(self.ref.pk)
        self.assertEqual(DistanceCache.objects.count(), 0)

        # No patch this time: ensure_cached() runs for real and, under the
        # synchronous test task runner, fills the table before the read.
        cached = stats.finds_by_bearing(self.ref.pk)
        self.assertGreater(DistanceCache.objects.count(), 0)
        self.assertEqual(fallback, cached)


class Build360MissingWhereSqlTests(TestCase):
    """build_360_missing_where_sql()'s fallback branch for caches without a
    DistanceCache row yet must compose correctly with the found/completed
    filter and the existing use_corrected=False PK re-add branch.
    ...
    def setUp(self):
        ...
        # Unfound, due north (bearing 0), ~56 km away: inside both the
        # requested sector {0} and max_km=100.
        _cache("GC1", found=False, latitude=48.5, longitude=9.0)
        # Unfound, due south, ~2200 km away: outside max_km (and sector).
        _cache("GC2", found=False, latitude=28.0, longitude=9.0)
        # Unfound, due east (bearing ~90): inside max_km but not sector {0}.
        _cache("GC3", found=False, latitude=48.0, longitude=9.7)
        # Found, otherwise a perfect match: must never appear.
        _cache("GC4", found=True, latitude=48.4, longitude=9.0)
    ...
    def _select(self, sql):
        ...
        from geocaches.filtering.query import apply_all

        qd = QueryDict(urlencode({"where_sql": sql}))
        qs, fv = apply_all(Geocache.objects.all(), qd)
        self.assertEqual(fv.get("where_error"), "", sql)
        return set(qs.values_list("gc_code", flat=True))

    def test_selects_right_caches_with_empty_distance_cache(self):
        with patch("geocaches.geo.distance_cache.ensure_cached", return_value=False):
            sql = stats.build_360_missing_where_sql(self.ref.pk, 100.0, [0])
        self.assertEqual(DistanceCache.objects.count(), 0)
        self.assertEqual(self._select(sql), {"GC1"})

    def test_agrees_with_cached_path_once_populated(self):
        with patch("geocaches.geo.distance_cache.ensure_cached", return_value=False):
            sql_fallback = stats.build_360_missing_where_sql(self.ref.pk, 100.0, [0])
        fallback_codes = self._select(sql_fallback)
        self.assertEqual(DistanceCache.objects.count(), 0)
        self.assertEqual(fallback_codes, {"GC1"})

        # No patch: ensure_cached() fills the table for real this time.
        sql_cached = stats.build_360_missing_where_sql(self.ref.pk, 100.0, [0])
        self.assertGreater(DistanceCache.objects.count(), 0)
        self.assertEqual(self._select(sql_cached), fallback_codes)

    def test_uncached_corrected_coord_cache_honours_use_corrected_toggle(self):
        """A solved-but-unfound cache with corrected coordinates and no
        DistanceCache row must be judged by whichever coordinates
        use_corrected asks for, not by whichever ones the fallback branch
        happens to match at first."""
        from geocaches.models import CorrectedCoordinates

        # Original coords far south (don't qualify); corrected coords due
        # north close by (qualify on sector {0} / max_km=100).
        corrected_only = _cache("GC5", found=False, latitude=28.0, longitude=9.0)
        CorrectedCoordinates.objects.create(geocache=corrected_only, latitude=48.5, longitude=9.0)

        # Original coords due north close by (qualify); corrected coords far
        # south (don't qualify).
        original_only = _cache("GC6", found=False, latitude=48.5, longitude=9.0)
        CorrectedCoordinates.objects.create(geocache=original_only, latitude=28.0, longitude=9.0)

        with patch("geocaches.geo.distance_cache.ensure_cached", return_value=False):
            sql_original = stats.build_360_missing_where_sql(
                self.ref.pk, 100.0, [0], use_corrected=False
            )
            sql_corrected = stats.build_360_missing_where_sql(
                self.ref.pk, 100.0, [0], use_corrected=True
            )
        self.assertEqual(DistanceCache.objects.count(), 0)

        codes_original = self._select(sql_original)
        self.assertIn("GC6", codes_original)     # original coords qualify
        self.assertNotIn("GC5", codes_original)  # only the corrected coords qualify

        codes_corrected = self._select(sql_corrected)
        self.assertIn("GC5", codes_corrected)     # corrected coords qualify
        self.assertNotIn("GC6", codes_corrected)  # only the original coords qualify
...
class AdventureLabStatsTests(TestCase):
    ...
    def setUp(self):
        ...
        Geocache.objects.create(  # parent row carries the country
            al_code="LCAAA", name="A", cache_type=CacheType.LAB,
            adventure=self.adv_a, is_al_parent=True,
            iso_country_code="DE", completed=False,
        ...
        Geocache.objects.create(
            ...
            adventure=self.adv_b, is_al_parent=True, iso_country_code="US",
    ...
    def test_unstarted_adventure_counts_as_not_started(self):
        ...
        Geocache.objects.create(
            ...
            adventure=adv, is_al_parent=True, latitude=48.0, longitude=9.0,
        )
...
class RegionMapDataTests(TestCase):
    ...
    def test_normalize_name_rules(self):
        ...
        # DK — stored "Region Hovedstaden" joins the polygon's bare "Hovedstaden".
        self.assertEqual(n("Region Hovedstaden", "DK"), n("Hovedstaden", "DK"))
        self.assertEqual(n("Region Sjælland", "DK"), n("Sjælland", "DK"))
        # DK counties — "Varde Kommune" joins "Varde"; the Danish capital names
        # join the polygon's English "Copenhagen".
        self.assertEqual(n("Varde Kommune", "DK"), n("Varde", "DK"))
        self.assertEqual(n("København", "DK"), n("Copenhagen", "DK"))
        self.assertEqual(n("Københavns Kommune", "DK"), n("Copenhagen", "DK"))
