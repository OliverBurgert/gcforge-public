...
class TestExtractAddressFields(unittest.TestCase):
    ...
    def test_denmark_municipality_beats_village(self):
        """Denmark has no ``county`` key; the Kommune is the county-level unit
        and the village Nominatim also returns must not be stored as county."""
        addr = {
        ...
        self.assertEqual(_extract_address_fields(addr)["county"], "Stevns Kommune")

    def test_denmark_municipality_beats_city(self):
        addr = {
        ...
        self.assertEqual(_extract_address_fields(addr)["county"], "Københavns Kommune")
