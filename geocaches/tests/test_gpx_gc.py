...
class TestParseCacheFields(TestCase):
    ...
    def test_parse_trackable_mentions(self):
        tb_xml = (
            '<groundspeak:travelbug id="6886842" ref="TB7WZ44" '
            'xmlns:groundspeak="http://www.groundspeak.com/cache/1/0/1">'
            '<groundspeak:name>Scotty Duck Geocoin</groundspeak:name>'
            '</groundspeak:travelbug>'
            '<groundspeak:travelbug id="9999" ref="TB9ZZZZ" '
            'xmlns:groundspeak="http://www.groundspeak.com/cache/1/0/1">'
            '<groundspeak:name>Second Bug</groundspeak:name>'
            '</groundspeak:travelbug>'
        )
        _, cache_el = _make_wpt(travelbugs_xml=tb_xml)
        mentions = parse_trackable_mentions(cache_el)
        self.assertEqual(len(mentions), 2)
        self.assertEqual(mentions[0], {"gc_id": 6886842, "ref_code": "TB7WZ44", "name": "Scotty Duck Geocoin"})
        self.assertEqual(mentions[1]["ref_code"], "TB9ZZZZ")

    def test_parse_trackable_mentions_empty(self):
        _, cache_el = _make_wpt(travelbugs_xml="")
        self.assertEqual(parse_trackable_mentions(cache_el), [])
...
class TestParseWptsGpx(TestCase):
    ...
    def test_single_waypoint_linked_to_parent(self):
        ...
        self.assertEqual(wpt["prefix"], "")  # "P1" is not a valid type prefix
    ...
    def test_valid_type_prefix_stored(self):
        """GSAK-style prefixes that match a known type are preserved."""
        for lookup, expected_prefix in [
            ("PK12345", "PK"), ("ST12345", "ST"), ("QA12345", "QA"),
            ("FL12345", "FL"), ("TH12345", "TH"), ("RP12345", "RP"), ("WP12345", "WP"),
        ]:
            with self.subTest(lookup=lookup):
                path = self._write_wpts_file(_make_wpts_gpx([_make_wpt_xml(lookup=lookup)]))
                result = parse_wpts_gpx(path)
                self.assertEqual(result["GC12345"][0]["prefix"], expected_prefix)

    def test_numeric_prefix_stored_blank(self):
        """GC.com numeric prefixes like '01' are discarded — type is derived from sym."""
        for lookup in ("0112345", "0212345", "9912345"):
            with self.subTest(lookup=lookup):
                path = self._write_wpts_file(_make_wpts_gpx([_make_wpt_xml(lookup=lookup)]))
                result = parse_wpts_gpx(path)
                self.assertEqual(result["GC12345"][0]["prefix"], "")

    def test_unrecognised_alpha_prefix_stored_blank(self):
        """Mixed / unknown prefixes like 'P1', 'S2' are not stored."""
        for lookup in ("P112345", "S212345", "XX12345"):
            with self.subTest(lookup=lookup):
                path = self._write_wpts_file(_make_wpts_gpx([_make_wpt_xml(lookup=lookup)]))
                result = parse_wpts_gpx(path)
                self.assertEqual(result["GC12345"][0]["prefix"], "")
...
class TestSaveGeocache(TestCase):
    ...
    def test_upserts_and_prunes_trackable_mentions(self):
        from geocaches.models import CacheTrackableMention

        mentions = [
            {"gc_id": 1, "ref_code": "TB1AAAA", "name": "Bug One"},
            {"gc_id": 2, "ref_code": "TB2BBBB", "name": "Bug Two"},
        ]
        save_geocache(self._basic_fields(), [], [], [], tb_mentions=mentions)
        cache = Geocache.objects.get(gc_code="GC99999")
        self.assertEqual(
            set(cache.trackable_mentions.values_list("ref_code", flat=True)),
            {"TB1AAAA", "TB2BBBB"},
        )

        # Re-import with only one of the two — the missing one should be pruned.
        save_geocache(
            self._basic_fields(), [], [], [],
            tb_mentions=[{"gc_id": 1, "ref_code": "TB1AAAA", "name": "Bug One Renamed"}],
        )
        rows = list(cache.trackable_mentions.all())
        self.assertEqual(len(rows), 1)
        self.assertEqual(rows[0].ref_code, "TB1AAAA")
        self.assertEqual(rows[0].name, "Bug One Renamed")
        self.assertEqual(CacheTrackableMention.objects.count(), 1)
