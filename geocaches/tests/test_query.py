"""
Tests for geocaches/filtering/query.py — reusable query helpers.
...
from geocaches.filtering.query import (
    BEARING_RANGES,
    annotate_distance,
    apply_action_scope,
    apply_all,
...
class TestMineFinderQ(TestCase):
    def test_no_accounts_returns_false_q(self):
        q, has = mine_finder_q()
    ...
    def test_matches_by_user_id(self):
        ...
        Log.objects.create(geocache=cache, log_type="Found it", user_id="42",
                           user_name="alice", source="gc", logged_date=date(2024, 1, 1))
        Log.objects.create(geocache=cache, log_type="Found it", user_id="99",
                           user_name="bob", source="gc", logged_date=date(2024, 1, 1))
        q, has = mine_finder_q()
        self.assertTrue(has)
        self.assertEqual(Log.objects.filter(q).count(), 1)

    def test_matches_by_username(self):
        from accounts.models import UserAccount
        from geocaches.models import Log
        cache = _make_cache("GC00001")
        UserAccount.objects.create(platform="oc_de", username="alice", user_id="")
        Log.objects.create(geocache=cache, log_type="Found it", user_id="",
                           user_name="alice", source="oc_de", logged_date=date(2024, 1, 1))
        Log.objects.create(geocache=cache, log_type="Found it", user_id="",
                           user_name="bob", source="oc_de", logged_date=date(2024, 1, 1))
        q, has = mine_finder_q()
        self.assertTrue(has)
        self.assertEqual(Log.objects.filter(q).count(), 1)
    ...
    def test_combines_multiple_accounts(self):
        ...
        cache = _make_cache("GC00001")
        UserAccount.objects.create(platform="gc", username="alice", user_id="42")
        UserAccount.objects.create(platform="oc_de", username="bob", user_id="99")
        Log.objects.create(geocache=cache, log_type="Found it", user_id="42",
                           user_name="alice", source="gc", logged_date=date(2024, 1, 1))
        Log.objects.create(geocache=cache, log_type="Found it", user_id="99",
                           user_name="bob", source="oc_de", logged_date=date(2024, 1, 1))
        Log.objects.create(geocache=cache, log_type="Found it", user_id="0",
                           user_name="charlie", source="gc", logged_date=date(2024, 1, 1))
        q, has = mine_finder_q()
        self.assertTrue(has)
        self.assertEqual(Log.objects.filter(q).count(), 2)
...
class TestApplyFilters(TestCase):
    ...
    def test_all_filter_chain_functions_called(self):
        """Verify FILTER_CHAIN is actually iterated — patch a survivor.

        After v2 phase 4d-ii cleanup, only the URL params that don't fit the
        pure-Q tree compiler still flow through FILTER_CHAIN.  Use
        apply_quick_search (handles ``?q=``) as a stand-in.
        """
        from geocaches.filtering import filters as _f
        original = _f.apply_quick_search
        calls = []
        ...
        _f.apply_quick_search = spy
        from geocaches.filtering import query as _q
        _q.FILTER_CHAIN[_q.FILTER_CHAIN.index(original)] = spy
        try:
            apply_filters(Geocache.objects.all(), {"q": "anything"})
            self.assertTrue(len(calls) > 0)
        finally:
            _q.FILTER_CHAIN[_q.FILTER_CHAIN.index(spy)] = original
            _f.apply_quick_search = original
...
class TestApplyActionScope(TestCase):
    """Tests for the action-bar 'target' picker narrowing.

    These narrow the queryset of caches an *action* affects (export, bulk
    tag, enrich, update, delete) without changing what the list view shows.
    Wired into _filtered_qs in geocaches/views/list.py — every action
    endpoint that goes through _filtered_qs inherits this.
    """

    def setUp(self):
        # Five caches along the diagonal lat/lon = (1,1)…(5,5)
        self.c1 = _make_cache("GC10001", lat=1.0, lon=1.0)
        self.c2 = _make_cache("GC10002", lat=2.0, lon=2.0)
        self.c3 = _make_cache("GC10003", lat=3.0, lon=3.0)
        self.c4 = _make_cache("GC10004", lat=4.0, lon=4.0)
        self.c5 = _make_cache("GC10005", lat=5.0, lon=5.0)

    # --- target=filter / default --------------------------------------------

    def test_target_absent_returns_unchanged(self):
        qs = apply_action_scope(Geocache.objects.all(), {})
        self.assertEqual(qs.count(), 5)

    def test_target_filter_returns_unchanged(self):
        qs = apply_action_scope(Geocache.objects.all(), {"target": "filter"})
        self.assertEqual(qs.count(), 5)

    def test_target_unknown_returns_unchanged(self):
        # Defensive: unknown values must not crash and must not narrow
        qs = apply_action_scope(Geocache.objects.all(), {"target": "bogus"})
        self.assertEqual(qs.count(), 5)

    # --- target=viewport ----------------------------------------------------

    def test_viewport_with_valid_vbox_narrows_to_bbox(self):
        # bbox covering caches 2 and 3 (lat/lon both in 1.5..3.5)
        qs = apply_action_scope(
            Geocache.objects.all(),
            {"target": "viewport", "vbox": "1.5,1.5,3.5,3.5"},
        )
        codes = set(qs.values_list("gc_code", flat=True))
        self.assertEqual(codes, {"GC10002", "GC10003"})

    def test_viewport_with_empty_vbox_returns_unchanged(self):
        qs = apply_action_scope(
            Geocache.objects.all(),
            {"target": "viewport", "vbox": ""},
        )
        self.assertEqual(qs.count(), 5)

    def test_viewport_with_missing_vbox_returns_unchanged(self):
        qs = apply_action_scope(
            Geocache.objects.all(),
            {"target": "viewport"},
        )
    ...
    def test_viewport_with_malformed_vbox_returns_unchanged(self):
        # Non-numeric — graceful no-op rather than 500
        qs = apply_action_scope(
            Geocache.objects.all(),
            {"target": "viewport", "vbox": "not,a,bbox,really"},
        )
        self.assertEqual(qs.count(), 5)

    def test_viewport_intersects_with_existing_filters(self):
        """Viewport is an *additional* AND clause — it doesn't replace other filters.

        This is the key contract from the user (see docs/action-bar-and-scope.md):
        the vbox AND-combines with whatever region/filter narrowing is already in
        place. Here we simulate that by pre-narrowing the qs (as apply_all would).
        """
        pre_narrowed = Geocache.objects.filter(latitude__gte=2.5)  # caches 3, 4, 5
        qs = apply_action_scope(
            pre_narrowed,
            {"target": "viewport", "vbox": "1.5,1.5,3.5,3.5"},  # caches 2, 3
        )
        codes = set(qs.values_list("gc_code", flat=True))
        # Intersection: only cache 3 satisfies both
        self.assertEqual(codes, {"GC10003"})

    # --- target=page --------------------------------------------------------

    def test_page_with_valid_ids_narrows_to_pks(self):
        qs = apply_action_scope(
            Geocache.objects.all(),
            {"target": "page", "ids": f"{self.c2.pk},{self.c4.pk}"},
        )
        codes = set(qs.values_list("gc_code", flat=True))
        self.assertEqual(codes, {"GC10002", "GC10004"})

    def test_page_with_empty_ids_returns_empty(self):
        # target=page with no ids means the user picked Page scope but the
        # click interceptor failed to inject IDs — fail safe by returning none.
        qs = apply_action_scope(
            Geocache.objects.all(),
            {"target": "page", "ids": ""},
        )
    ...
    def test_page_with_missing_ids_returns_empty(self):
        qs = apply_action_scope(
            Geocache.objects.all(),
            {"target": "page"},
        )
    ...
    def test_page_with_malformed_ids_returns_empty(self):
        # Non-integer values must not crash — return empty instead.
        qs = apply_action_scope(
            Geocache.objects.all(),
            {"target": "page", "ids": "abc,def"},
        )
        self.assertEqual(qs.count(), 0)
...
class TestApplyAll(TestCase):
    ...
    def test_no_ref_annotates_distance_as_null(self):
        # When no ref is active, apply_all annotates distance_km / bearing_deg
        # with NULL so fx-tree conditions that reference those fields don't
        # trip a FieldError at qs.filter() time.  The annotation exists but
        # is null, so any subsequent distance/bearing filter matches nothing.
        qs, fv = apply_all(Geocache.objects.all(), {}, ref=None)
        row = qs.first()
        self.assertTrue(hasattr(row, "distance_km"))
        self.assertIsNone(row.distance_km)
        self.assertTrue(hasattr(row, "bearing_deg"))
        self.assertIsNone(row.bearing_deg)
