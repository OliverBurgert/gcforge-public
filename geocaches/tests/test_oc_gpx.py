...
class ImportOcGpxTests(TestCase):
    ...
    def test_import_oc_gpx_translates_attributes_to_acodes(self):
        """OC GPX attribute ids (GC-equiv + OC-internal) become A-codes."""
        attrs = (
            '<groundspeak:attributes xmlns:groundspeak="' + NS_GS + '">'
            # GC-equiv id 19 "Ticks" -> A64 "Look out for ticks"
            '<groundspeak:attribute id="19" inc="1">Ticks</groundspeak:attribute>'
            # GC-equiv id 62 inc=0 "Seasonal access" -> A44 "All seasons"
            '<groundspeak:attribute id="62" inc="0">Seasonal access</groundspeak:attribute>'
            # OC-internal id 110 "Active railway nearby" -> A60
            '<groundspeak:attribute id="110" inc="1">Active railway nearby</groundspeak:attribute>'
            '</groundspeak:attributes>'
        )
        path = _write_tmp(_full_oc_gpx(other_code="", attrs_xml=attrs))
        import_oc_gpx(path)

        cache = Geocache.objects.get(oc_code="OC13726")
        got = {
            (a.source, a.attribute_id, a.is_positive): a.name
            for a in cache.attributes.all()
        }
        self.assertEqual(got, {
            ("oc", 64, True): "Look out for ticks",
            ("oc", 44, False): "All seasons",
            ("oc", 60, True): "Active railway nearby",
        })
