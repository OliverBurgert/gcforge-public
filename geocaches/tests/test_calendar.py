...
class RemoveMissingDaysTests(TestCase):
    def setUp(self):
        cal.add_missing_days(CacheType.MULTI, minimum=1, days=30)
        cal.add_missing_days(CacheType.LETTERBOX, minimum=1, days=30)

    def _count(self, **kw):
        return CalendarEntry.objects.filter(
            kind=CalendarEntry.KIND_MISSING_DAY, **kw
        ).count()

    def test_removes_only_chosen_type(self):
        kept = self._count(cache_type=CacheType.LETTERBOX)
        res = cal.remove_missing_days(CacheType.MULTI)
        self.assertGreater(res["removed"], 0)
        self.assertEqual(self._count(cache_type=CacheType.MULTI), 0)
        # The other type's to-dos are untouched.
        self.assertEqual(self._count(cache_type=CacheType.LETTERBOX), kept)

    def test_empty_type_targets_any_bucket(self):
        cal.add_missing_days(None, minimum=1, days=30)  # "any cache" bucket
        self.assertGreater(self._count(cache_type=""), 0)
        res = cal.remove_missing_days(None)
        self.assertGreater(res["removed"], 0)
        self.assertEqual(self._count(cache_type=""), 0)
        # Typed buckets survive.
        self.assertGreater(self._count(cache_type=CacheType.MULTI), 0)

    def test_no_match_returns_zero(self):
        res = cal.remove_missing_days(CacheType.TRADITIONAL)
...
class ViewTests(TestCase):
    ...
    def test_remove_missing_endpoint(self):
        ...
        self.assertTrue(
            CalendarEntry.objects.filter(
                kind=CalendarEntry.KIND_MISSING_DAY, cache_type=CacheType.MULTI
            ).exists()
        )
        resp = self.client.post(reverse("geocaches:calendar_remove_missing"),
                                {"stat_type": CacheType.MULTI})
        self.assertEqual(resp.status_code, 200)
        self.assertContains(resp, "to-do")
        self.assertNotIn("HX-Trigger", resp)
        self.assertFalse(
