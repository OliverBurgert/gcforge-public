...
class TestSaveAlcStageFormatA(TestCase):
    def test_creates_stage_and_adventure(self):
        ...
        self.assertEqual(stage.al_detail.stage_number, 1)
    ...
    def test_uuid_lookup(self):
        ...
        stage_gc = Geocache.objects.create(
            al_code="LC28NG-1", name="Original", cache_type=CacheType.LAB,
            latitude=48.5, longitude=9.1, adventure=adv,
        )
        ALStageDetail.objects.create(geocache=stage_gc, stage_number=1, al_stage_uuid="aaaa-bbbb")
        row = _make_alc_row(
        ...
        self.assertEqual(result, "updated")
        stage = ALStageDetail.objects.get(al_stage_uuid="aaaa-bbbb").geocache
