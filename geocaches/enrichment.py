...
# City-states where the city name differs from the state, so the ``state ==
# city`` test below misses them: Washington DC (state "District of Columbia",
# city "Washington").  There the district (ward) is the sub-county unit, as for
# Berlin's Bezirke — store it so the district map can break the state down.
_DISTRICT_CITY_STATES = {"District of Columbia"}

# Countries with no ``county`` key where Nominatim's ``municipality`` is the
# county-tier unit ("Stevns Kommune") and any village/town/city it also returns
# is just a locality inside it — so the municipality must beat the
# kreisfreie-Stadt city fallback below.
_MUNICIPALITY_COUNTY_COUNTRIES = {"DK"}
...
def _extract_address_fields(addr: dict) -> dict[str, str]:
    ...
    # A real county / Landkreis when Nominatim has one.
    county = addr.get("county") or ""
    if (not county
            and (addr.get("country_code") or "").upper() in _MUNICIPALITY_COUNTY_COUNTRIES):
        county = addr.get("municipality") or ""
    if not county:
        # No Landkreis: distinguish a kreisfreie Stadt from a city-state.  A
        # kreisfreie Stadt (München, Leipzig, …) IS the county-level unit, so
        # use the city name — and it must beat any finer ``municipality`` /
        # Stadtteil Nominatim also attaches (it sometimes names a neighbouring
        # Gemeinde, e.g. "Lichtenau" for points inside Chemnitz).  A city-state
        # (Berlin/Hamburg/Bremen, where state == city, or DC) has nothing
        # between the state and its Bezirke/wards — use the Stadtbezirk /
        # Stadtteil / borough there.
        city = addr.get("city") or addr.get("town") or addr.get("village") or ""
        if city and city != state and state not in _DISTRICT_CITY_STATES:
            county = city
        else:
            county = (addr.get("municipality") or addr.get("city_district")
                      or addr.get("borough") or "")
...
def fetch_location(lat: float, lon: float) -> dict[str, str]:
    ...
    """
    from geocaches.geo.countries import is_latin, strip_admin_suffix
...
def enrich_geocache(cache, fields: set[str], overwrite: set[str] | None = None) -> bool:
    ...
    needs_location = (
        "location" in fields
        # Manual-location caches were edited by hand — never touch them, not
        # even in overwrite mode.  The user can clear the flag via the
        # detail-page edit dialog when they want enrichment back.
        and not cache.manual_location
        and (
            loc_overwrite
    ...
    if changed:
        ...
        # The global write lock prevents background tasks from contending on
        # the SQLite writer.  The retry loop is kept as a backstop in case a
        # foreground request (admin, management command) writes concurrently.
        ...
        for attempt in range(6):
            try:
                ...
                if "database is locked" not in str(exc) or attempt == 5:
                    raise
...
def enrich_queryset(
    ...
    # Materialise PKs before the save loop so no open SELECT cursor holds a
    # SQLite shared lock during writes.  An open iterator cursor + a concurrent
    # write on the same connection causes SQLITE_LOCKED (which ignores the
    # 30 s timeout) resulting in "database is locked" errors mid-run.
    pk_list = list(to_enrich.values_list("pk", flat=True))
    CacheModel = qs.model
    ...
    for pk in pk_list:
        ...
        try:
            cache = CacheModel.objects.get(pk=pk)
            if enrich_geocache(cache, fields, overwrite):
                stats.updated += 1
            else:
                stats.no_data += 1
        except Exception as exc:
            stats.errors.append(f"{pk}: {exc}")
            logger.warning("Error enriching %s: %s", pk, exc)
        count += 1
    ...
    if stats.updated:
        from geocaches.runtime_cache import invalidate_filter_dialog_options
