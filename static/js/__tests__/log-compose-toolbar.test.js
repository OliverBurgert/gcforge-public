 ...
 * (static/js/log-compose-toolbar.js, extracted from templates/geocaches/
 * partials/_log_compose_toolbar.html — see
...
beforeAll(() => {
  ...
  const code = readFileSync(resolve(jsDir, 'log-compose-toolbar.js'), 'utf8')
  ;(0, eval)(code)
})
...
// ── top-level functions exist (onclick handlers) ──────────────────────────────

describe('log-compose-toolbar.js top-level functions', () => {
  const names = [
    'gcfFmtWrap', 'gcfFmtInsert', 'gcfFmtInsertLine', 'gcfFmtLines',
    'gcfFmtNumbered', 'gcfFmtInsertTemplate', 'gcfFmtLink',
  ]
...
describe('gcfFmtLink', () => {
  it('wraps the selection as a Markdown link using the prompted URL', () => {
    const ta = makeTextarea({ value: 'my cache', start: 3, end: 8 }) // "cache"
    const origPrompt = window.prompt
    window.prompt = () => 'https://example.com'
    globalThis.gcfFmtLink()
    window.prompt = origPrompt
    expect(ta.value).toBe('my [cache](https://example.com)')
  })

  it('does nothing when the prompt is cancelled', () => {
    const ta = makeTextarea({ value: 'my cache', start: 3, end: 8 })
    const origPrompt = window.prompt
    window.prompt = () => null
    globalThis.gcfFmtLink()
    window.prompt = origPrompt
    expect(ta.value).toBe('my cache')
  })

  it('uses the literal "link" as the label when there is no selection', () => {
    const ta = makeTextarea({ value: '', start: 0, end: 0 })
    const origPrompt = window.prompt
    window.prompt = () => 'https://example.com'
    globalThis.gcfFmtLink()
    window.prompt = origPrompt
    expect(ta.value).toBe('[link](https://example.com)')
  })
})
