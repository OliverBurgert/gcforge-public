...
describe('gcfBuildCurrentListUrl', () => {
  ...
  // Regression: adding a toolbar Country filter on top of an "or"-root tree
  // (e.g. the "show all with missing information" menu filter) used to
  // splice the new condition into the same "or" group instead of ANDing it
  // onto the whole thing, so it widened the result set instead of narrowing it.
  it('ANDs a toolbar condition onto an "or"-root fx tree instead of joining it as another branch', () => {
    const orTree = {
      g: 'or',
      c: [
    ...
    setupListPage({ fx: globalThis.gcfEncodeFx(orTree) })
    document.querySelector('[data-fx-country]').value = 'DE'
    const next = globalThis.gcfBuildCurrentListUrl()
    const fx = new URL('http://localhost' + next.replace(/^[^?]+/, '')).searchParams.get('fx')
    const tree = globalThis.gcfDecodeFx(fx)

    expect(tree.g).toBe('and')
    const countryCond = tree.c.find(c => c.f === 'country')
    expect(countryCond).toEqual({ f: 'country', op: 'in', v: ['DE'] })
    const orGroup = tree.c.find(c => c.g === 'or')
    expect(orGroup).toEqual(orTree)
  })
})
