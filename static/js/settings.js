...
/* ---- Reference-points / offline map init ---- */
(function() {
  var cfg = document.getElementById('settings-config');
  if (!cfg || typeof gcfLoadMapLibre !== 'function') return;
  gcfLoadMapLibre({
    scripts: [
      cfg.dataset.mapStylesUrl,
      cfg.dataset.settingsMapUrl,
      cfg.dataset.offlineMapUrl
    ],
    onReady: function() {
      if (typeof gcfRpMapInit === 'function') gcfRpMapInit();
      // If page loaded with #offline already active, initialise map now
      if ((window.location.hash || '') === '#offline' && typeof gcfOfflineTabShown === 'function') {
        gcfOfflineTabShown();
      }
    }
  });
