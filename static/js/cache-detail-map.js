...
function gcfDetailMapInit() {
  ...
  _gcfDetailState = {
    ...
    updateLocationUrl: cfg.dataset.updateLocationUrl,
    updateElevationUrl: cfg.dataset.updateElevationUrl,
...
function _gcfDetailAddCacheLayer() {
  ...
  // Background click — left-click menu (but not during a draw mode, and not
  // on the click that just finalised a draw).
  _gcfDetailMap.on('click', function(e) {
    ...
    // The menu-close listener below is also bound to document 'click' (so a
    // later click elsewhere dismisses the menu) — without stopping
    // propagation here, this very click would bubble to document and close
    // the menu in the same tick it just opened.
    if (e.originalEvent) e.originalEvent.stopPropagation();
    _gcfDetailShowBackgroundMenu(e);
  });
}
...
// ── Background (left-click) menu ────────────────────────────────────────────
// Styled like the right-click context menu (shares its DOM element + CSS
// classes — see "Right-click context menu" below).

function _gcfDetailShowBackgroundMenu(e) {
  ...
  header.className = 'map-ctx-coord-header';
  header.textContent = latStr + ', ' + lonStr;
  _gcfDetailCtxMenuEl.appendChild(header);

  _gcfDetailCtxMenuEl.appendChild(_gcfDetailMenuItem(gettext('Add waypoint here'), function() {
  ...
  _gcfDetailCtxMenuEl.appendChild(_gcfDetailMenuSeparator());

  _gcfDetailCtxMenuEl.appendChild(_gcfDetailMenuItem(gettext('Update country/state/county from here'), function() {
    _gcfDetailUpdateFromPoint(_gcfDetailState.updateLocationUrl, lat, lng);
  }));

  _gcfDetailCtxMenuEl.appendChild(_gcfDetailMenuItem(gettext('Update elevation from here'), function() {
    _gcfDetailUpdateFromPoint(_gcfDetailState.updateElevationUrl, lat, lng);
  }));

  _gcfDetailPositionCtxMenu(e.point);
}

function _gcfDetailUpdateFromPoint(url, lat, lng) {
  if (!url) return;
  ...
  fetch(url, {
    ...
    headers: {
      'Content-Type': 'application/x-www-form-urlencoded',
      'X-CSRFToken': _gcfDetailState.csrfToken
    },
    body: body.toString()
    ...
    .then(function(data) {
      if (data.ok) {
        ...
        _gcfDetailFlashMessage(data.error || gettext('Update failed.'));
      }
    })
    .catch(function() {
      _gcfDetailFlashMessage(gettext('Update failed.'));
    });
}

function _gcfDetailFlashMessage(text) {
...
function _gcfDetailSetupDraw() {
  ...
  _gcfDetailMap.on('mousemove', function(e) {
    ...
    _gcfDetailTipShow(e.point, 'segment ' + _gcfFmtDistSafe(seg) + ' · ' + label + ' ' + _gcfFmtDistSafe(total));
  });
...
function _gcfDetailRectTip(start, cur, point) {
  ...
  _gcfDetailTipShow(point, _gcfFmtDistSafe(w) + ' × ' + _gcfFmtDistSafe(h));
}
...
function _GcfDetailDrawCircle() {
  ...
  m.onDrag = function(state, e) {
    ...
    _gcfDetailTipShow(e.point, 'radius ' + _gcfFmtDistSafe(r));
  };
  m.onMouseMove = function(state, e) {
...
function _gcfDetailSetupContextMenu() {
  ...
  function _showCtx(e) {
    ...
    _gcfDetailCtxMenuEl.appendChild(_gcfDetailMenuItem(gettext('Street View'), function() {
      window.open('https://www.google.com/maps/@?api=1&map_action=pano&viewpoint=' +
        lat.toFixed(6) + ',' + lng.toFixed(6), '_blank');
    }));

    _gcfDetailCtxMenuEl.appendChild(_gcfDetailMenuItem(gettext('Range circle…'), function() {
...
// ── Shared menu item / positioning helpers (right-click + left-click menus) ──

function _gcfDetailMenuItem(label, handler) {
  var btn = document.createElement('button');
  btn.className = 'map-draw-context-item';
...
function _gcfDetailMenuSeparator() {
  var sep = document.createElement('div');
  sep.className = 'map-ctx-separator';
  return sep;
}

function _gcfDetailPositionCtxMenu(point) {
