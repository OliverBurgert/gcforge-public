// ── fx URL encoding (urlsafe-base64 of JSON, matches filtering/filter_expr.py) ─────────
...
// Inverse of _gcfReadToolbarConditions(): set every visible filter widget in
// the toolbar (the data-fx-* dropdowns, search box, elevation, radius, center
// location and saved-filter picker) to match `url`. The widgets live outside
// the swapped table container, so after an in-place change a stale value (e.g.
// a tag whose chip was just removed, or search text after Reset) would keep
// showing and be re-applied the next time the filter-form submits or the map
// refetches markers (gcfOnMapMoveForScope, _gcfBuildFilterParams).
...
function _gcfSyncToolbarFromUrl(url) {
  ...
  ['q', 'elevation', 'radius'].forEach(function (name) {
    var el = document.querySelector('#filter-form [name="' + name + '"]');
    if (el) el.value = sp.get(name) || '';
  });

  // No (or unknown) ?ref= means the server falls back to the default ref point.
  var refEl = document.querySelector('#filter-form [name="ref"]');
  if (refEl) {
    refEl.value = sp.get('ref') || '';
    if (refEl.selectedIndex < 0) {
      var defOpt = refEl.querySelector('option[data-default]') || refEl.options[0];
      if (defOpt) defOpt.selected = true;
    }
  }

  var savedSel = document.getElementById('saved-filter-select');
  if (savedSel) {
    var savedName = sp.get('f') || '';
    var savedIdx = 0;
    for (var s = 0; savedName && s < savedSel.options.length; s++) {
      if (savedSel.options[s].dataset.savedName === savedName) { savedIdx = s; break; }
    }
    savedSel.selectedIndex = savedIdx;
    _gcfSyncSavedFilterControls(savedSel);
  }
...
// Combine `kept` (existing non-toolbar-owned conditions/groups, in the
// existing tree's original root op) with `toolbarConds` (the dropdowns'
// current values). Toolbar conditions must always AND onto the result —
// each dropdown is an independent constraint, never an alternative — so
// when the existing root is "or" (e.g. the "show missing info" menu filter:
// `state is_none OR county is_none OR elevation is_null`), it has to be
// wrapped as its own subgroup rather than have toolbarConds appended
// straight into its c[], which would turn them into extra OR branches
// instead of ANDing onto the whole group.
function _gcfMergeToolbarConditions(tree, kept, toolbarConds) {
  var op = tree.g || 'and';
  if (!toolbarConds.length) return { g: op, c: kept };
  if (!kept.length) return { g: 'and', c: toolbarConds };
  if (op === 'and') return { g: 'and', c: kept.concat(toolbarConds) };
  return { g: 'and', c: [{ g: op, c: kept }].concat(toolbarConds) };
}
...
function gcfToolbarConfigureRequest(evt) {
  ...
  var merged = _gcfMergeToolbarConditions(tree, kept, toolbarConds);
...
function gcfBuildCurrentListUrl() {
  ...
  var merged = _gcfMergeToolbarConditions(tree, kept, _gcfReadToolbarConditions());
  var encoded = gcfEncodeFx(merged);
...
// Point _savedFilterSelectedPk and the delete button at the picker's current
// selection. Returns the selected <option> (may be the placeholder).
function _gcfSyncSavedFilterControls(selectEl) {
  var opt = selectEl.options[selectEl.selectedIndex];
  var hasValue = !!(opt && opt.value);
  _savedFilterSelectedPk = hasValue ? opt.value : null;
  var deleteBtn = document.getElementById('saved-filter-delete-btn');
  if (deleteBtn) deleteBtn.classList.toggle('d-none', !hasValue || opt.dataset.builtin === '1');
  return opt;
}

function gcfLoadSavedFilter(selectEl) {
  var opt = _gcfSyncSavedFilterControls(selectEl);
...
(function () {
  // WP-04: the dialog body (incl. #tree-json) is fetched lazily and may not
  // exist yet — on show.bs.modal (fires synchronously, before the lazy fetch
  // resolves) this silently no-ops on the first open; the matching
  // htmx:oobAfterSwap listener below re-runs it once the content arrives.
  var dlg = document.getElementById('treeFilterDialog');
  if (!dlg) return;
  function populateTreeDialog() {
    var ta = document.getElementById('tree-json');
    if (!ta) return;
    // Same staleness issue as the tabbed dialog: rebuild from the address
    // bar's ?fx= instead of the page-load-time {{ current_tree_json }},
    // which never gets refreshed by an in-place htmx filter change.
    var currentFx = (new URLSearchParams(window.location.search)).get('fx') || '';
    var tree = gcfDecodeFx(currentFx);
    ta.value = JSON.stringify(tree, null, 2);
    _gcfTreeShowError('');
  }
  dlg.addEventListener('show.bs.modal', populateTreeDialog);
  document.body.addEventListener('htmx:oobAfterSwap', function (evt) {
    if (evt.target && evt.target.id === 'treeFilterDialogContent') populateTreeDialog();
  });
})();
...
function _gcfTabbedFormToTree() {
  ...
  // Found day/month (all years) — separate widget pair from the per-field
  // mode picker above; day and/or month, matched across every year.
  var dmDayEl = form.elements['found_date_dm_day'];
  var dmMonthEl = form.elements['found_date_dm_month'];
  var dmDay = dmDayEl && dmDayEl.value.trim() ? parseInt(dmDayEl.value, 10) : null;
  var dmMonth = dmMonthEl && dmMonthEl.value ? parseInt(dmMonthEl.value, 10) : null;
  if (dmDay || dmMonth) {
    var dmV = {};
    if (dmDay) dmV.day = dmDay;
    if (dmMonth) dmV.month = dmMonth;
    var dmCond = { f: 'found_date', op: 'day_month', v: dmV };
    var dmNegEl = form.elements['found_date_dm_negate'];
    conditions.push(dmNegEl && dmNegEl.checked ? { g: 'not', c: [dmCond] } : dmCond);
  }
  ...
  // Found accuracy — "" (Unknown) is a real option here, so don't use
  // _gcfTabReadSelectedValues(): its !!v filter would silently drop it.
  var faSel = form.elements['found_accuracy'];
  if (faSel) {
    var faVals = Array.prototype.map.call(faSel.selectedOptions, function (o) { return o.value; });
    if (faVals.length) conditions.push({ f: 'found_accuracy', op: 'in', v: faVals });
  }
...
function _gcfTabbedFormFromTree(tree) {
  ...
  function trySetFoundDayMonthCond(cond, negate) {
    if (cond.f !== 'found_date' || cond.op !== 'day_month') return false;
    var v = cond.v || {};
    var dayEl = form.elements['found_date_dm_day'];
    var monthEl = form.elements['found_date_dm_month'];
    if (dayEl && v.day) dayEl.value = v.day;
    if (monthEl && v.month) monthEl.value = v.month;
    if (negate) { var negEl = form.elements['found_date_dm_negate']; if (negEl) negEl.checked = true; }
    return true;
  }
  ...
  (tree.c || []).forEach(function (node) {
    ...
    if (node.g === 'not' && node.c && node.c.length === 1) {
      ...
      if (!inner.g && trySetFoundDayMonthCond(inner, true)) return;
    ...
    if (trySetFoundDayMonthCond(node, false)) return;
    ...
    if (f === 'found_accuracy' && op === 'in' && Array.isArray(v)) {
      var faSelPre = form.elements['found_accuracy'];
      if (faSelPre) v.forEach(function (val) {
        Array.prototype.forEach.call(faSelPre.options, function (opt) { if (opt.value === val) opt.selected = true; });
      });
      return;
    }
...
function _gcfTabbedTabCounts() {
  ...
  var dmDayCount = form.elements['found_date_dm_day'];
  var dmMonthCount = form.elements['found_date_dm_month'];
  if ((dmDayCount && dmDayCount.value.trim()) || (dmMonthCount && dmMonthCount.value)) counts.dates++;
  ...
  var faSelCount = form.elements['found_accuracy'];
  if (faSelCount && faSelCount.selectedOptions.length) counts.adv++;
...
(function () {
  // WP-04: the dialog body (the tabbed form) is fetched lazily; see the same
  // note in the treeFilterDialog IIFE above. _gcfTabbedFormFromTree /
  // _gcfTabbedTabCounts already no-op gracefully when #tabbed-filter-form
  // doesn't exist yet, so populateTabbedDialog is safe to call before the
  // lazy fetch resolves.
  var tfd = document.getElementById('tabbedFilterDialog');
  if (!tfd) return;
  function populateTabbedDialog() {
    // Decode fresh from the address bar rather than the page-load snapshot
    // in #tfd-current-tree: an htmx-driven filter change (toolbar dropdown,
    // chip removal, tabbed/JSON dialog apply, …) updates the URL via
    // pushState but never re-renders this dialog, so the script tag goes
    // stale the moment anything changes the filter without a full reload.
    var currentFx = (new URLSearchParams(window.location.search)).get('fx') || '';
    var tree = gcfDecodeFx(currentFx);
    _gcfTabbedFormFromTree(tree);
    _gcfTabbedUpdateBadges(_gcfTabbedTabCounts());
  }
  tfd.addEventListener('show.bs.modal', populateTabbedDialog);
  tfd.addEventListener('change', function () {
    _gcfTabbedUpdateBadges(_gcfTabbedTabCounts());
  });
  document.body.addEventListener('htmx:oobAfterSwap', function (evt) {
    if (evt.target && evt.target.id === 'tabbedFilterDialogContent') populateTabbedDialog();
  });
})();
...
(function () {
  // WP-04: the dialog body (incl. #wc-where-sql) is fetched lazily; see the
  // same note in the treeFilterDialog IIFE above.
  var dlg = document.getElementById('whereClauseDialog');
  if (!dlg) return;
  function populateWhereDialog() {
    // Same staleness issue as the other two dialogs: {{ fv.where_sql }} only
    // reflects the page-load-time request, not a later in-place htmx update.
    var taSql = document.getElementById('wc-where-sql');
    var hName = document.getElementById('wc-where-name');
    var p = new URLSearchParams(window.location.search);
    if (taSql) taSql.value = p.get('where_sql') || '';
    if (hName) hName.value = p.get('where_name') || '';
    _gcfWcShowError('');
  }
  dlg.addEventListener('show.bs.modal', populateWhereDialog);
  document.body.addEventListener('htmx:oobAfterSwap', function (evt) {
    if (evt.target && evt.target.id === 'whereClauseDialogContent') populateWhereDialog();
  });
})();
...
// ── Action bar: Full update confirmation ─────────────────────────────────────
// templates/geocaches/partials/_action_bar.html — hx-swap-oob'd on every
// filter/scope change, so this lives in the host page's own script instead
// of an inline <script> that would re-run on every swap.
function gcfConfirmFullUpdate(el) {
  var n = parseInt(el.dataset.count || '0', 10);
  var apiCalls = n + Math.ceil(n / 25);
  var mins = Math.max(1, Math.round(n / 30));
  return confirm(
    gettext("Full update will refresh") + ' ' + n + ' ' + gettext("cache(s).") + '\n\n' +
    gettext("Estimated API usage: ~") + apiCalls + ' ' + gettext("calls (~2 per GC cache: data batch + per-cache logs)") + '\n' +
    gettext("Estimated time: ~") + mins + ' ' + gettext("minute(s)") + '\n\n' +
    gettext("This may consume significant GC API quota. Proceed?")
  );
}
