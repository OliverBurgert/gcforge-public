...
(function () {
  ...
  // --- MapLibre map init ---
  if (typeof gcfLoadMapLibre === 'function' && cfg) {
    gcfLoadMapLibre({
      draw: true,
      scripts: [
        cfg.dataset.mapStylesUrl,
        cfg.dataset.mapOfflineUrl,
        cfg.dataset.mapIconsUrl,
        cfg.dataset.mapDetailUrl
      ],
      onReady: function() {
        if (typeof gcfDetailMapInit === 'function') gcfDetailMapInit();
      }
    });
  }

  // --- ALC tag-change propagate-to-stages confirmation ---
  (function () {
    var modalEl = document.getElementById('alcTagPropagateModal');
    if (!modalEl || !cfg) return;
    var modal = new bootstrap.Modal(modalEl);
    var pendingForm = null;

    var tagEditPath = cfg.dataset.tagEditUrl;
    document.querySelectorAll('form').forEach(function (form) {
      if (form.getAttribute('action') === tagEditPath) {
    ...
    document.getElementById('alc-tag-yes').addEventListener('click', function () {
      modal.hide();
    ...
    document.getElementById('alc-tag-no').addEventListener('click', function () {
      modal.hide();
      if (!pendingForm) return;
      pendingForm.submit();
      pendingForm = null;
