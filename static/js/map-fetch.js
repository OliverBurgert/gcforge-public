...
function gcfFetchSetMode(mode) {
  ...
  if (isCriteria) {
    ...
    _gcfUpdateShapeLimitRow();
  }
}

// Show/hide the "search only in drawn shape" checkbox: only relevant (and
// only rendered) when the user actually has a shape drawn to limit to.
function _gcfUpdateShapeLimitRow() {
  var regions = (typeof _gcfDrawRegions !== 'undefined') ? _gcfDrawRegions : [];
  var row = document.getElementById('crit-limit-shape-row');
  if (row) row.style.display = regions.length ? '' : 'none';
}
...
// Strip a drawn-region object down to the plain shape fields the backend
// expects (drops MapLibre-draw feature ids etc). Shared by "by area" and by
// the "search only in drawn shape" criteria path below.
function _gcfRegionToShape(r) {
...
// Set after each preview submit so the post-sync "pin as filter" step (in
// _gcfSyncComplete) knows whether the search was actually bounded by the
// drawn regions — true for "by area" always, and for "by criteria" only
// when "search only in drawn shape" was checked.
var _gcfLastPreviewUsedRegions = false;
...
function gcfSubmitPreview() {
  ...
  if (_gcfFetchMode === 'criteria') {
    ...
    var regions = (typeof _gcfDrawRegions !== 'undefined') ? _gcfDrawRegions : [];
    var shapeEl = document.getElementById('crit-limit-shape');
    var useShape = regions.length > 0 && shapeEl && shapeEl.checked;

    if (useShape) {
      // One criteria search per drawn shape — same fan-out "by area" uses
      // for multiple regions, just with the criteria attached to each.
      regionData = regions.map(function(r) {
        var shape = _gcfRegionToShape(r);
        return shape ? { type: 'criteria', criteria: criteria, shape: shape } : null;
      }).filter(Boolean);
      if (!regionData.length) {
        _gcfFlashMessage(gettext('No previewable regions'));
        return;
      }
      _gcfLastPreviewUsedRegions = true;
    } else {
      ...
      _gcfLastPreviewUsedRegions = false;
    }
  } else {
    var regions = (typeof _gcfDrawRegions !== 'undefined') ? _gcfDrawRegions : [];
    if (!regions.length) {
    ...
    regionData = regions.map(_gcfRegionToShape).filter(Boolean);
    if (!regionData.length) {
      _gcfFlashMessage(gettext('No previewable regions'));
      return;
    }
...
function _gcfPollPreviewTasks(taskIds) {
  ...
  function _checkAllDone() {
    if (resultsDone === totalTasks) {
      _gcfPreviewComplete(allCaches, allErrors, anyCancelled);
    }
  }
      ...
      fetch('/tasks/' + tid + '/')
        ...
        .then(function(info) {
          ...
          if (info.state === 'completed' || info.state === 'failed' || info.state === 'cancelled') {
            ...
            if (info.state === 'cancelled') anyCancelled = true;

            if (info.state === 'completed' || info.state === 'cancelled') {
              // A cancelled task may still carry results — the fetch can finish
              // (fully or partially) just after the cancel request goes out.
...
function _gcfPreviewComplete(caches, errors, cancelled) {
  document.getElementById('fetch-preview-bar').style.width = '100%';
  errors = errors || [];

  if (!caches.length) {
    var noResultMsg = cancelled
      ? gettext('Preview cancelled — no caches were fetched yet')
      : (_gcfFetchMode === 'criteria')
  ...
  var flashMsg = interpolate(
    cancelled
      ? gettext('Preview cancelled — %(total)s caches fetched so far (%(newCount)s new, %(existCount)s already synced)')
...
function _gcfSyncComplete(created, updated, failed) {
  ...
  // area. Skip the rewrite when the search wasn't actually bounded by a drawn
  // shape (worldwide/criteria-only or view-limited "by criteria" searches).
  var regions = (!_gcfLastPreviewUsedRegions || typeof _gcfDrawRegions === 'undefined')
    ? [] : _gcfDrawRegions;
