...
function fbOpen(targetInputName, extensions, multiSelect, startDirOverride) {
  ...
  document.querySelector("#fileBrowserModal .modal-title").textContent =
    _fbMultiSelect ? gettext("Select file(s) or folder") : gettext("Select file");

  // Reset UI
  _fbUpdateFooter();
  document.getElementById("fb-select-btn").disabled = true;

  // Use explicit override dir, or derive from current input value
  var startDir = startDirOverride || "";
  if (!startDir) {
    ...
    .then(function(resp) {
      if (!resp.ok) {
        return resp.json().then(function(data) {
          throw new Error(data.error || gettext("Request failed"));
        });
      }
      return resp.json();
    })
...
function _fbUpdateFooter() {
  ...
  if (_fbMultiSelect && _fbSelectedPaths.size > 1) {
    display.textContent = interpolate(
      ngettext("%s file selected", "%s files selected", _fbSelectedPaths.size),
      [_fbSelectedPaths.size]);
  } else if (_fbSelectedPath) {
...
function _fbRenderListing(entries) {
  ...
  if (!entries || entries.length === 0) {
    listing.innerHTML = '<div class="text-muted small p-2">' + gettext("Empty directory") + '</div>';
    return;
  }
  ...
  if (_fbMultiSelect) {
    html += '<div class="p-2 border-bottom">'
      ...
      + '&#128193; ' + gettext('Select this folder') + '</button></div>';
  }
