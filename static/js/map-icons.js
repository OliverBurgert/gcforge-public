 ...
 * Format: "i-{typeShort}-{gc|oc}-{statusKey}" with optional "-dnf", "-c" and
 * "-s" (Adventure Lab stage — inverted colours) suffixes.
 */
function gcfMapIconId(typeShort, platform, statusKey, dnf, corrected, stage) {
  return 'i-' + typeShort + '-' + platform + '-' + statusKey + (dnf ? '-dnf' : '') + (corrected ? '-c' : '') + (stage ? '-s' : '');
}
...
/**
 * Composite a same-origin SVG image into a solid colour, using its alpha
 * channel as a mask (source-in). Used to draw the Adventure Lab stage glyph
 * in purple instead of its native white.
 */
function _gcfTintImage(img, w, h, color) {
 ...
 *
 * `stage` draws the Adventure Lab "inverted" variant (white fill, purple ring,
 * purple glyph) so a stage is visually distinct from its parent. Only applied
 * to the unfound branch — a found stage still renders as the golden smiley.
 */
function _gcfGenerateIcon(typeShort, platform, statusKey, size, stage) {
  size = size || 32;
  ...
  // Adventure Lab stage: inverted colours (white fill, purple ring, purple
  // glyph below) instead of the parent's solid-purple disc — replaces any
  // status-colour override (archived/disabled) so a stage always reads as a
  // stage first; the archived overlay below still runs on top of the white
  // fill. The ring goes to 2px (vs. the usual 1.5px) — a colour-on-white ring
  // needs more weight than the usual dark outline to stay legible at 32px.
  var ringWidth = 1.5;
  if (stage) {
    fillColor = '#ffffff';
  ...
  if (svgImg && svgImg.naturalWidth > 0 && svgImg.naturalHeight > 0) {
    try {
      ...
      if (stage) {
        // Glyph artwork is pure white — tint it to the same purple as the
        // ring so it reads against the white fill.
        var tinted = _gcfTintImage(svgImg, iconSize, iconSize, typeInfo.color);
        ctx.drawImage(tinted, iconOffset, iconOffset, iconSize, iconSize);
      } else {
 ...
 */
function gcfEnsureMapIcon(map, typeShort, platform, statusKey, dnf, corrected, stage) {
  var id = gcfMapIconId(typeShort, platform, statusKey, dnf, corrected, stage);
  if (!_gcfIconCache[id]) {
    var imgData = _gcfGenerateIcon(typeShort, platform, statusKey, undefined, stage);
...
function gcfPrepareMapIcons(map, markers) {
  ...
  for (var i = 0; i < markers.length; i++) {
    ...
    var corr = m.ola != null;
    var dnf = !!m.dnf && !m.f;
    // Adventure Lab stage: has an adventure id but isn't the flagged parent.
    // Must be part of the icon identity or a parent and a stage would share
    // whichever image was generated first.
    var stage = m.aid != null && !m.alp;
    var key = m.t + '|' + plat + '|' + sk + '|' + dnf + '|' + corr + '|' + stage;
    if (!needed[key]) {
      needed[key] = { type: m.t, platform: plat, statusKey: sk, dnf: dnf, corrected: corr, stage: stage };
    }
  }
  var keys = Object.keys(needed);
  for (var j = 0; j < keys.length; j++) {
    var n = needed[keys[j]];
    gcfEnsureMapIcon(map, n.type, n.platform, n.statusKey, n.dnf, n.corrected, n.stage);
  }
}
