...
function _gcfShowTooltip(point, w_m, h_m) {
  ...
  _gcfTooltipEl.textContent = h_m !== undefined
    ? _gcfFmtDistRound(w_m) + ' \u00d7 ' + _gcfFmtDistRound(h_m)
    : _gcfFmtDistRound(w_m);
...
GcfDrawCircle.onSetup = function(opts) {
  var circle = this.newFeature({
    type: 'Feature',
    properties: { shape: 'circle', is_alc: _gcfNextCircleIsAlc ? 'true' : 'false' },
    geometry: { type: 'Polygon', coordinates: [[]] }
  });
...
GcfDrawCircle.onStop = function(state) {
  ...
  if (!state.committed) {
    ...
    if (_gcfNextCircleIsAlc) {
      // ALC drawing was cancelled — exit ALC mode entirely
      _gcfNextCircleIsAlc = false;
      _gcfDrawModeGroup = null;
      _gcfUpdateDrawModeUI();
    }
  }
};
...
var _gcfNextCircleIsAlc = false; // set true before starting an ALC-fetch circle draw
var _gcfDrawModeGroup = null;    // 'alc' | 'regular' | null — mutual exclusion between ALC and regular tools
...
function gcfDrawInit() {
  ...
  function _gcfAddCorridorBufferLayers() {
    ...
    // ALC circle label source + layer
    gcfMap.addSource('gcf-alc-labels', {
      type: 'geojson',
      data: { type: 'FeatureCollection', features: [] }
    });
    gcfMap.addLayer({
      id: 'gcf-alc-labels',
      type: 'symbol',
      source: 'gcf-alc-labels',
      layout: {
        'text-field': 'ALC fetch area',
        'text-size': 12,
...
function _gcfOnDrawCreate(e) {
  (e.features || []).forEach(function(f) {
    ...
    if (p.shape === 'rect' && p.bbox) {
      ...
      var circleType = _gcfNextCircleIsAlc ? 'al_circle' : 'circle';
      _gcfNextCircleIsAlc = false;
      _gcfDrawRegions.push({ type: circleType, center: center, radius_m: Number(p.radius_m), id: f.id });
      if (circleType === 'al_circle' && _gcfDrawModeGroup === 'alc') {
        // Auto-restart so the user can draw another ALC circle without clicking the button again
        setTimeout(function() {
          if (_gcfDrawModeGroup === 'alc' && _gcfDrawCtrl) {
            _gcfNextCircleIsAlc = true;
...
function _gcfUpdateDrawStatus() {
  ...
  if (typeof gcfUpdateAlcButtons === 'function') gcfUpdateAlcButtons();

  _gcfUpdateAlcLabels();
}

function _gcfUpdateAlcLabels() {
  if (!gcfMap || !gcfMap.getSource('gcf-alc-labels')) return;
  var features = [];
  _gcfDrawRegions.forEach(function(r) {
    if (r.type !== 'al_circle') return;
    // center is stored as [lat, lon]; GeoJSON coordinates are [lon, lat]
    features.push({
      type: 'Feature',
      geometry: { type: 'Point', coordinates: [r.center[1], r.center[0]] },
      properties: {}
    });
  });
  gcfMap.getSource('gcf-alc-labels').setData({ type: 'FeatureCollection', features: features });
}

function _gcfIsInAnyRegion(lat, lon) {
  for (var i = 0; i < _gcfDrawRegions.length; i++) {
    var r = _gcfDrawRegions[i];
    if (r.type === 'al_circle') continue; // ALC circles are for AL fetch, not cache filtering
    if (r.type === 'rect') {
...
function _gcfUpdateDrawModeUI() {
  var isAlc     = _gcfDrawModeGroup === 'alc';
  var isRegular = _gcfDrawModeGroup === 'regular';

  // Regular draw buttons: disabled while ALC mode is active
  ['map-draw-rect-btn', 'map-draw-circle-btn', 'map-draw-polygon-btn',
   'map-draw-corridor-btn', 'map-draw-gpx-btn'].forEach(function(id) {
    var btn = document.getElementById(id);
    if (btn) btn.disabled = isAlc;
  });

  // ALC button: disabled while a regular draw tool is active; shows active state in ALC mode
  var alcBtn = document.getElementById('map-draw-al-circle-btn');
  if (alcBtn) {
    alcBtn.disabled = isRegular;
    alcBtn.classList.toggle('active', isAlc);
  }

  // Filter group (badge + Pin/Save/Preview/Sync): hidden in ALC mode
  var filterGroup = document.getElementById('map-draw-filter-group');
  if (filterGroup) filterGroup.style.display = isAlc ? 'none' : 'contents';

  // Refresh ALC button visibility (accounts for _gcfDrawModeGroup in map-fetch.js)
  if (typeof gcfUpdateAlcButtons === 'function') gcfUpdateAlcButtons();
}
...
function gcfDrawAlCircle() {
  if (!_gcfDrawCtrl) return;
  if (_gcfDrawModeGroup === 'alc') {
    // Toggle off ALC mode
    _gcfNextCircleIsAlc = false;
    _gcfDrawModeGroup = null;
    _gcfDrawCtrl.changeMode('simple_select');
...
function _gcfSetActiveDrawBtn(activeId) {
  ...
  // Only register auto-deactivation handler when entering a draw mode (not on explicit off)
  if (activeId && gcfMap) {
    gcfMap.once('draw.modechange', function(e) {
      ['map-draw-rect-btn', 'map-draw-circle-btn', 'map-draw-polygon-btn', 'map-draw-corridor-btn'].forEach(function(id) {
        var btn = document.getElementById(id);
        if (btn) btn.classList.remove('active');
      });
      // Only exit regular mode when returning to idle (simple_select).
      // When switching between tools, modechange fires for the NEW tool's mode —
      // don't reset then or the ALC button briefly re-enables mid-transition.
      if (e.mode === 'simple_select' && _gcfDrawModeGroup === 'regular') {
        _gcfDrawModeGroup = null;
        _gcfUpdateDrawModeUI();
      }
    });
...
// Re-sync the drawn area shapes to the current ?geo= URL param. _gcfRestoreGeoShapes
// only runs once at draw-control init; an in-place list change (no full reload)
// that removes or changes the area filter must call this so the pinned shapes —
// and the "Pin as filter" status bar — follow the URL instead of going stale.
window.gcfSyncGeoShapes = function() {
  if (!_gcfDrawCtrl) return;
  _gcfDrawCtrl.deleteAll();   // fires draw.delete → prunes _gcfDrawRegions + status
  _gcfDrawRegions = [];       // belt-and-suspenders if the event is async
  _gcfRestoreGeoShapes();     // re-add shapes from the new ?geo= (no-op if absent)
  _gcfUpdateDrawStatus();     // hide the status bar when nothing remains
};
...
function gcfPinAsFilter() {
  ...
  // In-place where possible — gcfApplyListChange re-syncs the drawn shapes to
  // the new ?geo= and refreshes the markers without rebuilding MapLibre.
  if (typeof gcfApplyListChange === 'function') gcfApplyListChange(url);
...
function gcfSaveArea() {
  if (!_gcfDrawRegions.length) {
    alert(gettext('Draw shapes on the map first.'));
    return;
  }
  var name = prompt(gettext('Save area filter as:'));
  ...
  .then(function(data) {
    if (data.error) { alert(interpolate(gettext('Error: %s'), [data.error])); return; }
    _gcfLoadSavedAreas();
  })
  .catch(function(e) { alert(interpolate(gettext('Save failed: %s'), [e])); });
}
...
function _gcfRenderSavedAreas(areas) {
  ...
  if (!areas || !areas.length) {
    ...
    empty.textContent = gettext('No saved areas');
  ...
  areas.forEach(function(area) {
    ...
    nameSpan.title = interpolate(gettext('Load %s'), [area.name]);
...
function gcfLoadSavedArea(area) {
  ...
  params.delete('page');
  var url = window.location.pathname + '?' + params.toString();
  // In-place where possible — gcfApplyListChange draws the saved area's shapes
  // (via gcfSyncGeoShapes) and refreshes the markers without a full reload.
  if (typeof gcfApplyListChange === 'function') gcfApplyListChange(url);
  else window.location.search = params.toString();
}

function gcfDeleteSavedArea(id, name) {
  if (!confirm(interpolate(gettext('Delete saved area "%s"?'), [name]))) return;
  var csrfToken = document.querySelector('[name=csrfmiddlewaretoken]');
  ...
  .catch(function(e) { alert(interpolate(gettext('Delete failed: %s'), [e])); });
}
...
function gcfGpxFileChanged(input) {
  ...
  reader.onload = function(ev) {
    try {
      ...
      var lines = [
        interpolate(
          ngettext('%(segments)s segment, %(points)s points, ~%(km)s km',
                   '%(segments)s segments, %(points)s points, ~%(km)s km',
                   info.segmentCount),
          {
            segments: info.segmentCount,
            points: info.originalCount.toLocaleString(),
            km: info.lengthKm.toFixed(1)
          }, true)
      ];
      if (info.originalCount > 5000) {
        lines.push('<span class="text-warning">' + gettext('Large track \u2014 rendering may be slow') + '</span>');
      }
      if (infoEl) { infoEl.innerHTML = lines.join('<br>'); infoEl.style.display = ''; }
      if (importBtn) importBtn.disabled = false;
    } catch (err) {
      if (infoEl) {
        infoEl.textContent = interpolate(gettext('Error: %s'), [err.message]);
...
// Add a corridor region from an ordered [[lng,lat],...] path. Shared by the
// GPX-track importer and the route planner (map-route.js): registers the
// LineString with MapboxDraw, tracks it in _gcfDrawRegions, refreshes the
// buffer/boxes/status, and (unless opts.fit === false) fits the map to it.
// Returns the region object, or null on failure.
function gcfAddCorridorRegion(path, widthM, opts) {
  opts = opts || {};
  if (!_gcfDrawCtrl || !path || path.length < 2) return null;
  ...
  var region = { type: 'corridor', path: path, width_m: widthM, id: ids[0] };
  _gcfDrawRegions.push(region);
  _gcfCorridorWidthM = widthM;
  var corridorInput = document.getElementById('map-corridor-width');
  if (corridorInput) corridorInput.value = (widthM / 1000).toFixed(1);

  _gcfUpdateDrawStatus();
  _gcfUpdateCorridorBuffer();
  _gcfUpdateCorridorBoxes();

  if (opts.fit !== false && gcfMap) {
...
// Remove a region previously added via gcfAddCorridorRegion. Programmatic
// _gcfDrawCtrl.delete() does not fire draw.delete, so we sync _gcfDrawRegions
// and refresh the buffer/boxes/status ourselves. Safe to call with a stale
// region (e.g. the user already trashed it) — the id simply matches nothing.
function gcfRemoveCorridorRegion(region) {
  if (!_gcfDrawCtrl || !region) return;
  try { _gcfDrawCtrl.delete([region.id]); } catch (e) { /* already gone */ }
  _gcfDrawRegions = _gcfDrawRegions.filter(function(r) { return r.id !== region.id; });
  _gcfUpdateDrawStatus();
  _gcfUpdateCorridorBuffer();
  _gcfUpdateCorridorBoxes();
}
...
function gcfImportGpxConfirm() {
  ...
  gcfAddCorridorRegion(path, widthM);
}
