...
function _applyFormat(fmt) {
  ...
  if (fmt === 'oc') {
    label.textContent = gettext('Opencaching');
    ...
    wptsHelp.textContent = gettext('Opencaching files include waypoints inline — no companion file needed.');
  } else if (fmt === 'gc') {
    label.textContent = gettext('geocaching.com');
