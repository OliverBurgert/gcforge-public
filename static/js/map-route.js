...
function gcfRouteTableRender() {
  ...
  if (summary) {
    summary.textContent = interpolate(
      ...
      { stops: data.rows.length, dist: _gcfFmtDistRound(data.total_m) }, true,
    );
  }
  ...
  data.rows.forEach(function(r, i) {
    ...
    tr.appendChild(_gcfRouteCell(_gcfFmtDistRound(r.along_m), 'text-end'));
    tr.appendChild(_gcfRouteCell(r.waypoint ? '—' : _gcfFmtDistRound(r.offset_m), 'text-end'));
