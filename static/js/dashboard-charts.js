...
(function () {
  ...
  function wireAttrSort() {
    var group = document.getElementById("attr-sort-group");
    if (!group) return;
    group.addEventListener("click", function (e) {
      var btn = e.target.closest("[data-attr-sort]");
      if (!btn) return;
      var sort = btn.dataset.attrSort;
      var tbody = document.querySelector("#attr-stat-table tbody");
      if (!tbody) return;
      var rows = Array.from(tbody.querySelectorAll("tr"));
      rows.sort(function (a, b) {
        if (sort === "alpha") return a.dataset.name.localeCompare(b.dataset.name);
        if (sort === "count") return parseInt(b.dataset.count, 10) - parseInt(a.dataset.count, 10);
        return a.dataset.code < b.dataset.code ? -1 : a.dataset.code > b.dataset.code ? 1 : 0;
      });
      rows.forEach(function (r) { tbody.appendChild(r); });
      group.querySelectorAll("[data-attr-sort]").forEach(function (b) {
        b.classList.toggle("active", b === btn);
      });
    });
  }
  ...
  window.gcfWireAttrSort = wireAttrSort;
})();
