...
var _gcfFetchSeq = 0;             // monotonic counter; stale completions are discarded
...
// Expand the compact marker payload into the marker objects every map module
// reads (m.c / m.la / m.t / m.aid / …).
//
// The endpoint (geocaches/views/map.py::map_markers) sends
// { fields: ['c','n','la',…], rows: [[…], …], count: N } — one array per
// marker whose values map positionally onto `fields`, rather than one object
// per marker. Absent values are null, and trailing nulls are trimmed server
// side, so a row may be shorter than `fields`: the columns it stops before are
// simply left unset, exactly as the old per-marker objects omitted them.
function _gcfDecodeMarkers(data) {
  ...
  for (var i = 0; i < rows.length; i++) {
    var row = rows[i];
    var n = row.length < nFields ? row.length : nFields;
    var m = {};
    for (var j = 0; j < n; j++) m[fields[j]] = row[j];
    out[i] = m;
  }
  return out;
}
...
function _gcfFetchMarkers(searchOverride) {
  ...
  // Snapshot comparison params NOW (before the async gap). Computing this
  // in the .then() callback reads window.location.search at completion time,
  // which may have advanced to a newer filter — causing the stale fetch to
  // record the wrong _gcfLastMarkerParams and silently suppress the next refresh.
  var filterSnapshot = _gcfFilterParams(searchOverride);
  ...
  fetch('/map/markers/?' + params, { signal: controller.signal })
    ...
    .then(function(data) {
      // Discard if a newer fetch was started — its result is more current.
      // This prevents the initial map-load fetch (all caches, no filter) from
      // overwriting a filter-change fetch that started and completed after it.
      if (mySeq < _gcfFetchSeq) return;
      var markers = _gcfDecodeMarkers(data);
      ...
      for (var i = 0; i < markers.length; i++) {
        _gcfPromoteCorrectedCoords(markers[i]);
      }
      _gcfMarkersData = markers;
      _gcfRenderMarkers(markers);
      // Record which filter state produced these markers (snapshotted pre-fetch
      // so window.location.search drift can't corrupt the record).
      _gcfLastMarkerParams = filterSnapshot;
...
function _gcfRenderMarkers(markers, opts) {
  ...
  if (useIcons) {
    ...
    gcfMap.addLayer({
      ...
      filter: ['!', ['has', 'point_count']],
      layout: {
        'icon-image': ['concat', 'i-', ['get', 'cacheType'], '-', ['get', 'platform'], '-', ['get', 'statusKey'], ['get', 'dnf'], ['get', 'corrected'], ['get', 'stage']],
        'icon-size': 1,
